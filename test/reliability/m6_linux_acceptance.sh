...
set -u
export TMPDIR=/tmp
QD="$HOME/a7-tree/target/debug/qd"
...
for i in 1 2 3; do
  ...
  "$QD" new "$n" >/dev/null 2>&1; echo "  new $n rc=$?"
done
...
LSN=$("$QD" ls --short 2>/dev/null | grep -c "${JAIL_PREFIX}s")
echo "--- qd ls (registry): $LSN sessions ---"; "$QD" ls 2>&1 | tail -5
ck "ls count = 3 (registry layer)" "[ ${LSN:-0} -eq 3 ]"
for i in 0 1 2; do ck "info resolves ${NAMES[$i]}" "\"$QD\" info \"${NAMES[$i]}\" >/dev/null 2>&1"; done
echo "--- kill s3 by name; s1+s2 survive (registry resolution among many) ---"
"$QD" kill --force "${NAMES[2]}" >/dev/null 2>&1; sleep 1
ck "s3 gone from registry" "! \"$QD\" ls --short 2>/dev/null | grep -q \"${JAIL_PREFIX}s3\""
ck "s1 survives in registry" "\"$QD\" ls --short 2>/dev/null | grep -q \"${JAIL_PREFIX}s1\""
ck "s2 survives in registry" "\"$QD\" ls --short 2>/dev/null | grep -q \"${JAIL_PREFIX}s2\""
for n in "${NAMES[0]}" "${NAMES[1]}"; do "$QD" kill --force "$n" >/dev/null 2>&1; done; sleep 1
REM=$("$QD" ls --short 2>/dev/null | grep -c "${JAIL_PREFIX}s" || true); REM=$(printf %s "$REM" | tr -dc 0-9)
