...
ts_qd() { ( cd "$TS_DIR" && "$BUN_BIN" src/index.ts "$@" ); }
...
( cd "$WORKDIR" && env QDRG_STUB_NAME="$NAME" "$QD_BIN" new "$NAME" --cwd "$WORKDIR" ) \
...
ts_qd ls --all > "$JAIL_ROOT/r2.out" 2> "$JAIL_ROOT/r2.err"; r2=$?
...
ts_qd info "$NAME" > "$JAIL_ROOT/r3.out" 2> "$JAIL_ROOT/r3.err"; r3=$?
...
ts_qd gc > "$JAIL_ROOT/r4.out" 2> "$JAIL_ROOT/r4.err"; r4=$?
...
ts_qd gc > "$JAIL_ROOT/r5.out" 2> "$JAIL_ROOT/r5.err"; r5=$?
