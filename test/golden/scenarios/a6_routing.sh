...
# backend routing (G-A3) against the REAL Rust sb binary, real zmx, and a STUB
...
# scan_for_name) keys on the registry NAME; sb passes --name <zmxname>, and the
...
# new_session <name> [--via NAME] [ANTHROPIC_VAR=val ...] — drive `sb new` in the
# jail with the stub claude. Caller-captured ANTHROPIC_* vars are passed inline
# (export-before-run, the F1 capture surface). Returns sb's exit code; stdout/err
...
new_session() {
  ...
  # The stub must report the same name sb passes as --name (== the session name).
  ( cd "$WORKDIR" \
...
# `sb new` with NO ANTHROPIC_* set and NO --via writes NO env file and the
...
if [ -f "$HOME/.sb/session-env/$GA1.env" ]; then
...
# failure (marks.jsonl as a DIRECTORY → open fails) and assert the ported `sb new`
...
# ===========================================================================
# G-A2 F1-wiring positive (PORT row): `sb new` with ANTHROPIC_BASE_URL set in the
...
if [ -f "$HOME/.sb/session-env/$GA2.env" ]; then
...
# file cannot be corrupted mid-`sb new`, so we exercise the REAL prefix SHAPE
...
PREFIX="{ . '$BROKEN'; } || { echo 'sb: env file source failed, aborting session' >&2; exit 97; }; rm -f -- '$BROKEN'; "
...
# differ) + a file-backend secret. `sb new --via ccr-a` → the stub env dump shows
...
# under the jailed sb config via `sb config set <key> <value>` so get_secret
...
if grep -q "absent-key" "$JAIL_ROOT/$GA3E.err" && grep -q "sb config set" "$JAIL_ROOT/$GA3E.err"; then
...
# string. Grep the zmx command line (history/list), sb stdout/stderr captures,
...
LEAK=0
# sb captures (all .out/.err under JAIL_ROOT).
if grep -rq "$SECRET_VAL" "$JAIL_ROOT"/*.out "$JAIL_ROOT"/*.err 2>/dev/null; then
  note_fail "G-A5 secret value leaked into an sb stdout/stderr capture"; LEAK=1
fi
