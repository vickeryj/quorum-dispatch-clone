...
mod tests {
    ...
    fn refuse_attended_zmx_fails_closed_on_unknown_count() {
        ...
        assert!(
            refuse_attended_zmx(true, Some(1)),
            "attended zmx → refuse (unchanged)"
        );
        assert!(
            refuse_attended_zmx(true, Some(5)),
            "attended zmx → refuse (unchanged)"
        );
        assert!(
            !refuse_attended_zmx(true, Some(0)),
            "OBSERVED-unattended zmx → deliver (unchanged)"
        );
        // THE T2 ARM: an unreadable count now REFUSES (M3 shipped this fail-open).
        assert!(
            refuse_attended_zmx(true, None),
            "zmx UNKNOWN count → refuse (fail-CLOSED, T2)"
        );
        // Embedded never refused, regardless of the (synthesized) client count.
        assert!(
            !refuse_attended_zmx(false, Some(3)),
            "embedded → never refuse"
        );
        assert!(
            !refuse_attended_zmx(false, Some(0)),
            "embedded → never refuse"
        );
    ...
    fn attended_zmx_refusal_message_is_honest_about_why() {
        ...
        assert!(
            m.contains("a human is attached (3 client(s))"),
            "observed-attach msg: {m}"
        );
        ...
        assert!(
            u.contains("UNREADABLE"),
            "unknown msg names unreadability: {u}"
        );
        assert!(
            u.contains("cannot be ruled out"),
            "unknown msg is honest about the gap: {u}"
        );
        assert!(
            !u.contains("client(s))"),
            "unknown msg must NOT fabricate a client count: {u}"
        );
        assert!(
            !u.contains("is attached"),
            "unknown msg must NOT assert an observed attach: {u}"
        );
    }
