...
pub struct NewParams {
    ...
    /// `pi/extension`: the control socket the in-process `quorum-lane` extension
    /// should serve, carried straight through to
    /// [`crate::provider::LaunchRequest::control_socket`] when the launch cmd is
    /// built.
    ///
    /// `None` on every other create, which leaves the assembled cmd
    /// byte-identical for every existing caller — the same posture `interactive`
    /// takes below. This create path is already provider-generic (it builds its
    /// cmd through `provider.launch_plan`), so carrying the caller's request
    /// here is all the mux-pane pipeline needs to host a CHANNELLED pi TUI:
    /// same claim, same preflight, same I6 verify, one more flag.
...
fn build_claude_command(deps: &NewDeps, params: &NewParams) -> String {
    ...
    let req = LaunchRequest {
        ...
        control_socket: params.control_socket.clone(),
    };
