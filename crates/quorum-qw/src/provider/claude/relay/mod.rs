...
//!
//! LIVES UNDER `provider/claude/` (harness-first reorg): this module was
//! `src/relay.rs` at the crate root, but the cc-relay channel it models is a
//! CLAUDE-only mechanism — its sole consumers are [`super::adoption`] (the PID
//! match feeds adoption's relay-liveness check) and [`self::http`] (the HTTP
//! transport that implements [`RelayContract`] against the real socket).
//! Neither codex, pi, nor opencode has a cc-relay analog, so this is not shared
//! transport — it belongs beside the one harness that owns it. `crate::relay::…`
//! keeps resolving via the crate-root re-export in `lib.rs`.
