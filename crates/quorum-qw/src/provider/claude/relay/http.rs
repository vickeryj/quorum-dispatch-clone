...
//!
//! LIVES IN qw, under `provider/claude/relay/` (qd/qw split, then the
//! harness-first reorg): the cc-relay channel is a CLAUDE mechanism — the
//! sidecar it talks to is the `bun run --cwd ~/.claude/channels/relay` MCP
//! server a claude session loads, and the port scan keys on that server's
//! `/health` record. ALL provider-specific code belongs in `quorum-qw`. It was
//! `dispatch::relay_http`, then `provider::claude::relay_http`, and now sits
//! beside the contract it implements as `provider::claude::relay::http` (this
//! module) alongside [`super::RelayContract`] (`provider::claude::relay`, née
//! the crate-root `src/relay.rs`). `claude/mod.rs` re-exports it as
//! `relay_http` and `dispatch` re-exports THAT under its old flat name, so
//! every `dispatch::relay_http::…` / `crate::relay_http::…` call site keeps
//! resolving (`relay_server` is the in-crate one, plus 9 integration tests).
//! Being here is also the precondition for the claude/pane lane's
//! `LaneOps::deliver`, which needs this client from inside qw.
