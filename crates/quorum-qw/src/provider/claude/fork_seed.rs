...
//!
//! LIVES IN qw (qd/qw split): this module is claude-only by its own first line, and
//! ALL provider-specific code belongs in `quorum-qw`. It was `dispatch::fork_seed` and
//! moved here as a pure relocation — `dispatch` re-exports it under the old flat name
//! so every `dispatch::fork_seed::…` call site keeps resolving.
