...
//!
//! LIVES IN qw, under `provider/claude/` (qd/qw split): this was the DEEPEST claude
//! coupling left in the `dispatch` lib, and ALL provider-specific code belongs in
//! `quorum-qw`. It is claude-specific in every layer, not just at the edges —
//! [`classify_session`] answers `NotApplicable` for any `provider != "claude-code"`;
//! [`RESTART_FLAGS`] are claude CLI flags; [`claude_loads_relay_channel`] requires
//! argv[0]'s filename to be literally `claude`; [`is_relay_sidecar`] matches an exact
//! `bun run --cwd ~/.claude/channels/relay` argv; and the `stop_hook_*` family speaks
//! the Claude Code Stop-hook settings protocol.
//!
//! It could not move until two of its imports did. It reached for
//! `crate::identity::SessionIdentity` and `crate::resolve::is_live_status`, and both
//! of those lived in `dispatch` — a qw module cannot import from qd (the edge is
//! qd → qw, never the reverse). `identity` moved into qw alongside this file, and
//! `is_live_status` moved down into the `quorum-core` LEAF beside the
//! [`SessionStatus`](crate::model::SessionStatus) it matches on, which is why the
//! import below now reads `crate::model` rather than `crate::resolve`. Nothing else
//! about the module changed. `dispatch` re-exports it under its old flat name, so all
//! ~24 symbols the `qd` binary and the integration lanes consume through
//! `dispatch::adoption::…` / `crate::adoption::…` keep resolving unchanged.
...
// `is_live_status` sits in `crate::model` beside `SessionStatus`, not in
// `dispatch::resolve` where it was written: it is a pure match on the enum, and a qw
// module cannot reach into qd. Same function, `resolve` re-exports it.
use crate::model::{is_live_status, RelayHealth, Session};
...
/// The failure-injection seam over [`register_pending_with_remover`]: lets a caller
/// substitute the prepared-record remover so the cleanup-failure branch above is
/// reachable from a test.
///
/// It USED to be `#[cfg(test)] pub(crate)`, which was enough while this module and its
/// only consumer — `dispatch::relay_server::mcp`'s shutdown-for-adoption rows — were
/// compiled into the SAME crate. They no longer are (this module moved to `quorum-qw`,
/// mcp stayed in `dispatch`), and `cfg(test)` is per-crate: when `dispatch`'s test build
/// pulls `quorum-qw` in as a dependency, qw is built WITHOUT `cfg(test)`, so a gated item
/// simply is not there. Widening it to a plain `pub` is what keeps that row compiling.
/// `#[doc(hidden)]` + the `_for_test` suffix keep it marked as what it is: a seam, not a
/// production entry point. Production calls [`register_pending`].
#[doc(hidden)]
pub fn register_pending_with_remover_for_test(
    state_dir: &Path,
    record: &AdoptRecord,
    remove_prepared: &dyn Fn(&Path) -> io::Result<()>,
) -> Result<PathBuf, String> {
