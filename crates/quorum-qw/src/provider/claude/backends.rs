...
//!
//! LIVES IN qw (qd/qw split): `--via` routing composes the env the CLAUDE launcher
//! hands its child — provider-specific by construction, and ALL provider-specific code
//! belongs in `quorum-qw`. It was `dispatch::backends` and moved here as a pure
//! relocation, next to the [`crate::launch`] env-file foundation it leans on;
//! `dispatch` re-exports it under the old flat name so every `dispatch::backends::…`
...
/// QD_HOME-honored via the caller's already-resolved `QdPaths::state_dir`).
