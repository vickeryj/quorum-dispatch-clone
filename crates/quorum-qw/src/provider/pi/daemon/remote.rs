...
//! in-daemon truth ([`crate::provider::pi::stdio::PiStdio`]) is the OTHER `PiRpc` impl — it
...
//! feeds it through [`crate::provider::pi::status::PiStatusMapper`] into the registry sink
...
use crate::provider::pi::stdio::rpc::{PiEvent, PiRpc, PiRpcError, RpcSessionState, StreamingBehavior};
...
/// ls/info gather (`join` (dispatch-side)) renders a live pi row's status + turns off
