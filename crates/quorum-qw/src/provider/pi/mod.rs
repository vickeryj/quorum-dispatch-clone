...
//!
//! Submodules — HARNESS at this level, TRANSPORT one level down (the
//! provider-reorg discipline: `provider/pi/<transport>/`, never duplicated):
//!   - [`stdio`]    — the stdio JSONL-RPC transport to a `pi` child: the wire
//!     contract ([`stdio::rpc`]), the persistent daemon-internal driver
//!     ([`stdio::stdio`], [`PiStdio`]), and the unrelated one-shot
//!     `pi -p --mode json` NDJSON path ([`stdio::floor`]).
//!   - [`daemon`]   — the pi-daemon resident lane: the host process
//!     ([`daemon::residence`]) and the create/resume choreography that spawns
//!     and revives it ([`daemon::create`], [`daemon::resume`]), plus the verb-side
//!     ws client that reaches an already-running resident ([`daemon::remote`]).
//!   - [`pty`]      — the pi TUI hosted in a mux pane, for `qd start --interactive`
//!     ([`pty::pane`], [`pty::tui`]).
//!   - [`store`]    — read-only pi on-disk session reading: permissive
//!     session-JSONL reading + path math, tolerant of pi's lazy-write window
//!     ("no file" ≠ "no session") ([`store::session`]).
//!   - [`verify`]   — live-daemon verification harnesses (tier-a conformance,
//!     chaos, red-team) — test/evidence infrastructure, NOT a production path.
//!   - [`status`], [`pin`], [`republish`] — transport-neutral pi knowledge that
//!     answers to no one harness underneath: pure event→[`republish::Republish`]
//!     mapping, the version/protocol drift pin, and the pure Republish/Sink
//!     contract types. These stay flat; nothing about them is stdio- or
//!     daemon-specific.
//!
...
pub use extension::{Client as PiExtensionClient, ClientError as PiExtensionError};

pub use daemon::remote::{PiObservation, PiRemote};
pub use stdio::rpc::{PiEvent, PiRpc, PiRpcError, RpcSessionState, StreamingBehavior};

// ===========================================================================
// Compatibility re-exports — PRESERVE PRE-REORG MODULE PATHS.
//
// The provider reorg (PROVIDER-REORG-SPEC.md) moved every transport file one
// level deeper (`rpc.rs` -> `stdio/rpc.rs`, `daemon.rs` -> `daemon/create.rs`,
// etc.), but ~200 external call sites across `dispatch` still spell the OLD
// flat paths: `pi::rpc::X`, `pi::stdio::X`, `pi::floor::X`, `pi::residence::X`,
// `pi::remote::X`, `pi::resume::X`, `pi::pane::X`, `pi::tui::X`, `pi::session::X`,
// `pi::conformance::X`, `pi::chaos::X`, `pi::redteam::X`. Rather than touch that
// blast radius, each old name is re-bound here to its new home so both paths
// resolve to the same module. (`pi::daemon::X` needs no alias — `daemon` is
// still the name of the top-level submodule, it just gained internal structure;
// see `daemon/mod.rs`. `pi::stdio::PiStdio` likewise needs no alias — `stdio` is
// still the name of the top-level submodule, and its `mod.rs` re-exports
// `PiStdio` at its own root.)
pub use daemon::remote;
pub use daemon::residence;
pub use daemon::resume;
pub use pty::pane;
pub use pty::tui;
pub use stdio::floor;
pub use stdio::rpc;
pub use store::session;
pub use verify::chaos;
pub use verify::conformance;
pub use verify::redteam;
...
impl Provider for PiProvider {
    ...
    fn launch_plan(&self, fx: &ProviderFx, req: &LaunchRequest) -> LaunchPlan {
        ...
        let argv = if req.interactive {
            let mut argv = match req.resume.as_deref().filter(|s| !s.is_empty()) {
                Some(id) => vec!["--session-id".to_string(), id.to_string()],
                None => vec![],
            };
            // `pi/extension`: the ONE argv difference between this lane and
            // `pi/mux-pane`. The `quorum-lane` extension is inert until this
            // flag names a socket for it to serve, so emitting it here is what
            // turns an ordinary pi TUI into a drivable one. Absent on every
            // other launch, which is why no existing pi caller changes.
            if let Some(sock) = req.control_socket.as_deref().filter(|s| !s.is_empty()) {
                argv.push(extension::SOCK_FLAG.to_string());
    ...
    /// `InjectError::NoTransport`. pid is NEVER read.
    ///
    /// `from` is SPENT, not dropped (punch R10). A pi prompt is a plain string —
    /// the `prompt` frame has a `message` and no sender field — so the
    /// attribution rides in the TEXT: [`attribution::attribute`] puts the
    /// relay-shaped `<channel source="qd" from_session="…">…</channel>` envelope
    /// plus its one-line reply path in front of the message whenever `from` names
    /// a real peer, and passes the message through BYTE-IDENTICAL when it is
    /// `"cli"` (an operator shell is not a peer to answer). Before that this impl
    /// took the parameter as `_from` and a delegated task arrived here
    /// unattributed, with no hint that `qd send:relay` could answer it.
    ///
    /// The envelope changes only the TEXT — the busy probe, the `FollowUp`/`Steer`
    /// routing, the minted id and the error mapping are all untouched, and the
    /// ledger keys sha256 of the RAW message on both seams. What DOES move with
    /// it is the content-keyed landing check, which reads pi's rollout and
    /// therefore sees the WIRE text: [`crate::provider::pi::floor::rollout_landed`]
    /// un-wraps with [`attribution::inner_body`] before it hashes, so one key
    /// still joins the send record, the SEND-seam confirmation and the WAIT-seam
    /// terminal.
    fn inject(
        &self,
        fx: &ProviderFx,
        ...
        // when there is no sender to name.
...
mod tests {
    ...
    use crate::provider::pi::stdio::rpc::{PiEvent, RpcSessionState};
    ...
    struct RoutingRpc {
        ...
        /// The prompt TEXT each `inject` handed the rpc — the R10 envelope's witness.
        prompted: RefCell<Vec<String>>,
    }
    ...
    impl PiRpc for RoutingRpc {
        ...
        fn prompt(
            ...
            self.prompted.borrow_mut().push(m.to_string());
            Ok("c1".to_string())
        }
    ...
    fn route_with(state: Option<bool>) -> (Option<StreamingBehavior>, Result<String, InjectError>) {
        let (routed, out, _wire) = route_from(state, "cli");
        (routed, out)
    }

    /// The `inject` drive with an explicit SENDER, answering the WIRE text too —
    /// the routing tests all drive the `"cli"` (no-envelope) lane through
    /// [`route_with`]; the R10 envelope tests need the attributed one.
    fn route_from(
        state: Option<bool>,
        from: &str,
    ) -> (
        ...
        let wire = rpc.prompted.borrow().first().cloned();
        (rpc.routed(), out, wire)
    }

    /// punch R10 — an attributed send reaches the pi resident WRAPPED: the prompt
    /// carries the sender and the one-line reply path, and the body is the
    /// operator's message byte-for-byte. This is the exact string a pi session
    /// reads, so it is pinned verbatim.
    #[test]
    fn an_attributed_send_wraps_the_pi_prompt() {
        let (routed, out, wire) = route_from(Some(false), "sess-A");
        assert_eq!(
            wire.as_deref(),
            Some(
                "<channel source=\"qd\" from_session=\"sess-A\">\n\
                 the message\n\
                 </channel>\n\
                 [reply: qd send:relay sess-A \"<your reply>\" | peers: qd ls]"
            )
        );
        // TEXT only: the routing decision and the minted id are untouched.
    ...
    /// peer: no envelope, the message reaches the resident byte-identical.
    #[test]
    fn a_cli_send_reaches_the_resident_unwrapped() {
        let (_, out, wire) = route_from(Some(false), "cli");
        assert_eq!(wire.as_deref(), Some("the message"));
        assert_eq!(out.unwrap(), "c1");
    }

    /// The envelope rides on BOTH routings — a queued follow-up is as much a
    /// delegated message as a fresh turn, and an attribution that appeared only
    /// when the resident happened to be idle would be worse than none.
    #[test]
    fn a_busy_resident_gets_the_envelope_too() {
        let (routed, _, wire) = route_from(Some(true), "sess-A");
        assert_eq!(routed, Some(StreamingBehavior::FollowUp));
        assert!(wire
            .unwrap()
            .starts_with("<channel source=\"qd\" from_session=\"sess-A\">"));
    }
