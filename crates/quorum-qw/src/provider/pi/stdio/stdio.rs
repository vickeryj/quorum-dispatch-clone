...
//! `pi-daemon` resident — [`crate::provider::pi::daemon::residence`]). The resident owns ONE `PiStdio`
...
//! [`crate::provider::pi::status::PiStatusMapper`] into the registry sink, item 2).
//!
...
impl PiStdio {
    ...
    /// after).
    ///
    /// Spawned WITHOUT `process_group(0)` here: the *resident* ([`crate::provider::pi::daemon::residence`])
