...
//!
//! THE CODEX PROBLEM, AND WHY PI DOES NOT HAVE IT. [`crate::provider::codex::tui`]
...
/// needs is the flag, not a number: pinning to [`crate::provider::pi::pin::PINNED_VERSION`] would
...
/// rides, which is a different contract; see [`crate::provider::pi::pin`].)
///
...
pub fn session_id_is_taken(sessions_root: &Path, cwd: &str, id: &str) -> bool {
    ...
    crate::provider::pi::store::session::find_session_file(sessions_root, id, Some(&cwd)).is_some()
}
...
mod tests {
    ...
    fn write_session(root: &Path, cwd: &str, ts: &str, id: &str) {
        ...
        let dir = root.join(crate::provider::pi::store::session::encode_cwd_dir(&resolved));
