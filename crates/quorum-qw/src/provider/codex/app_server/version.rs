...
/// path is from THIS module: src/provider/codex/app_server/version.rs →
/// NOTE (qd/qw split): this file moved into `quorum-qw`, but the pin stays in
/// `dispatch/tests/fixtures/` because `dispatch/tests/codex_schema.rs` reads it
/// via `CARGO_MANIFEST_DIR` and must keep finding it there. Rather than
/// duplicate what this module calls the single source of truth — and invite
/// drift — the `include_str!` reaches across the crate boundary. Paths are not
/// crate-scoped, so this is legal; it is just unusual, hence this note.
/// tests/). Verified to compile against the committed fixture.
///
/// NOTE (harness-first reorg): this file relocated one level deeper
/// (`codex/version.rs` → `codex/app_server/version.rs`), so the relative
/// `include_str!` path below gained one more `../` to keep pointing at the
...
const fn include_pin() -> &'static str {
    let raw = include_str!("../../../../../dispatch/tests/fixtures/codex-schema/VERSION.pin");
    trim_ascii_end(raw)
}
