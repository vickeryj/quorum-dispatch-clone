...
//!
//! Split out of the former flat `src/create_daemon.rs` (PROVIDER-REORG-SPEC.md).
//! The spawn/port/cmdline-probe/reap PRIMITIVES this pipeline is built from
//! moved to [`crate::provider::shared::daemon`] — acp and pi drive the SAME
//! primitives for their own daemon lanes. What stayed HERE is everything that
//! is codex-specific: the `run_new_daemon` choreography itself, its
//! `DaemonError`/`DaemonDeps` surface, and [`cmdline_is_our_daemon`], which is
//! provably codex-only because it requires the literal `"codex"` AND
//! `"app-server"` argv tokens — pi and acp have their own
//! `cmdline_is_our_pi_daemon` / `cmdline_is_our_acp_daemon` built on the same
//! shared [`crate::provider::shared::daemon::CmdlineProbe`].
//!
...
//!   1. version sniff (§3.4) — WARN on any drift, then proceed. Breaking used to
//!      be a hard refusal gated on `QD_CODEX_UNPINNED=1`; that is now a warning
//!      by owner ruling (2026-08-17). See [`check_version`] for what was traded.
...
//! [`crate::provider::shared::daemon::DaemonSpawner`] (real + test fake); the
//! rpc arrives via an injected CONNECTOR closure ([`RpcConnector`]; tests pass
//! a fixture rpc, never a real socket); clock/env/exec are the house seams.
...
use crate::provider::shared::daemon::{DaemonSpawner, PortAllocator, SpawnedDaemon};
...
// falsely report "running". The probe itself
// ([`crate::provider::shared::daemon::CmdlineProbe`] +
// [`crate::provider::shared::daemon::real_cmdline_probe`]) is shared across
// every harness's daemon lane; what is codex-only is the MATCH below.
// ===========================================================================
...
pub struct DaemonParams {
    ...
    /// The `hosting` token to STAMP ON THE ROW — `Some("daemon")` for
    /// `codex/daemon`, `Some("app-server")` for `codex/app-server`.
    ///
    /// # Why this field is the whole difference between the two lanes
    ///
    /// The two lanes run the identical choreography and produce an identical
    /// process. What separates them is what the ROW says afterwards, because the
    /// row is what every later verb re-derives the lane from
    /// (`crate::provider::row_hosting`). A `codex/app-server` create that left
    /// this `None` would come back as `codex/daemon` on the very next call and
    /// its attach would be refused — the session would be correct and
    /// unaddressable at the same time.
    ///
    /// # Why `codex/daemon` now stamps too, having once passed `None`
    ///
    /// The old reasoning was that `row_hosting("codex", None)` already answers
    /// `Daemon`, so the token bought nothing. That was true, and it made the
    /// row's lane an INFERENCE — a fact recoverable only by reading a default
    /// that lives in a file the row never mentions. An unstamped row is
    /// indistinguishable from a row whose writer had not heard of lanes, and it
    /// re-derives through whatever that default says on the day it is read.
    /// Stamping makes the lane data. `Option` survives because the type still
    /// has to express "no opinion" for the readers of rows written before this
    /// change — see [`crate::lane::Harness::row_default_mode`], which answers
    /// for exactly those and is frozen so that they never move.
...
pub fn run_new_daemon<'a>(
    ...
    // Any drift → a WARNING; the create proceeds either way (read the override
...
fn check_version(deps: &DaemonDeps) -> Result<(), DaemonError> {
    match version::sniff(deps.exec) {
        ...
        SniffOutcome::Verdict(VersionVerdict::Breaking { found, pin }) => {
            // RULED 2026-08-17: drift WARNS, it does not refuse.
            //
            // What was traded. This used to be a hard refusal, and the refusal was
            // the only thing standing between a breaking codex release and a
            // corrupted session. It is now a line of stderr, which nobody reads.
            // The guard did not disappear so much as MOVE: the real detector is
            // `dispatch/scripts/codex-schema-diff.sh`, which regenerates the
            // app-server JSON schema and exits nonzero on any drift against the
            // committed fixture. That belongs in CI. This function only ever
            // compared THREE INTEGERS and never looked at the protocol at all.
            //
            // `QD_CODEX_UNPINNED=1` still means something: it silences the
            // warning, because a caller who set it has already acknowledged the
            // drift and does not need telling twice.
...
fn finish_create(
    ...
    Ok(DaemonOutcome {
        ...
        pid: spawned.pid,
        thread_id,
        qd_session_id: qd_session_id.to_string(),
...
mod tests {
    ...
    use crate::provider::shared::daemon::real_alloc_port;
    ...
    #[test]
    fn version_breaking_warns_and_proceeds() {
        let h = harness();
        let exec =
        ...
        // 0.140 against the 0.146 pin is a BREAKING verdict, and under the
        // 2026-08-17 ruling it no longer blocks: the create runs to completion.
        // The warning itself is stderr and deliberately not asserted here — this
        // pins the DECISION (proceed), not the wording.
        let out = run_new_daemon(&deps, &params("cdx", None)).expect("breaking drift must proceed");
        assert_eq!(out.pid, 4242);
        assert_eq!(spawner.spawn_count(), 1, "the version gate no longer blocks");
        assert!(
            h.sessions_dir.join("4242.json").exists(),
            "a proceeding create writes its row"
        );
    }
