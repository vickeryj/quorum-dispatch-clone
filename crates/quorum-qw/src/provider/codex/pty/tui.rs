//! `provider::codex::pty::tui` — identity discovery for a codex session running
//! as a TUI in a mux pane (`qd start <name> --provider codex --interactive`).
//!
...
// `rollout` moved to `store/rollout.rs` (a sibling of `pty/`, not of this file)
// during the harness-first reorg; the old bare `super::rollout` no longer
// resolves from here, so it is pulled in explicitly under its new canonical
// path and used unqualified below exactly as before.
use crate::provider::codex::store::rollout;
        ...
        for mo in months.flatten() {
            ...
            for d in days.flatten() {
                ...
                for f in files.flatten() {
                    ...
                    if rollout::parse_filename(&fname).is_none() {
                        continue;
                    }
                    let mtime_ms = f
                        .metadata()
...
fn read_session_meta(path: &Path) -> Option<RolloutCandidate> {
    ...
    let rec = rollout::parse_line(trimmed)?;
    let rollout::RolloutLine::SessionMeta { id, cwd } = rec.line else {
        return None;
    };
...
mod tests {
    ...
    fn epoch_and_known_instants_are_exact() {
        ...
        assert_eq!(
            parse_iso8601_ms("2000-03-01T00:00:00Z"),
