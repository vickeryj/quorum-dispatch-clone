...
mod tests {
    ...
    fn send_terminal_release_ordering() {
        ...
        assert!(
            q.try_release().is_none(),
            "no release while a turn is in flight (SC-1)"
        );
        ...
        let r2 = q
            .try_release()
            .expect("second releases after first terminal");
    ...
    fn wrong_turn_id_terminal_does_not_release() {
        ...
        assert_eq!(
            q.in_flight(),
            Some(t1.as_str()),
            "in-flight untouched by a wrong-turn terminal"
        );
        assert!(!q.is_idle());
    ...
    fn cancel_and_drain_discards_and_counts() {
        ...
        assert_eq!(
            out.cancel_turn,
            Some(t1.clone()),
            "names the in-flight turn to cancel"
        );
    ...
    fn cancel_and_drain_idle_session_has_no_cancel_turn() {
        ...
        let out = q.cancel_and_drain();
        assert_eq!(
            out.cancel_turn, None,
            "idle session: nothing in flight to cancel"
        );
    ...
    fn teardown_guaranteed_unblock() {
        ...
        assert_eq!(
            out.abandoned_turn,
            Some(t1),
            "teardown abandons the wedged in-flight turn"
        );
        assert_eq!(out.discarded, 1);
    ...
    fn turn_ids_are_unique_and_attributable_to_session() {
        ...
        assert_ne!(a, b);
        assert!(
            a.starts_with("sess-XYZ#"),
            "turn id is attributable to the session"
        );
    }
}
