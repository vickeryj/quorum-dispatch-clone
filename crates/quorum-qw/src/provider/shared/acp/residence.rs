//! `provider::acp::residence` — the cross-process ACP **residence layer**
//! (STEP0-RESIDENCE-SCOPE §2c seams S1/S2/S6/S8). This is the dispatch-side machinery
...
//!
//! LIVES IN qw (qd/qw split): this is provider-specific code — the ACP bridge's host
//! process — and ALL provider-specific code belongs in `quorum-qw`. It was
//! `dispatch::acp_residence` and moved here as a pure relocation; `dispatch` re-exports
//! the module under the old name so every `dispatch::acp_residence::…` call site keeps
//! resolving. Its structural twin [`super::super::pi::residence`] was already here.
//! NOTE on [`ADAPTER_VERB`]: it is BOTH a hidden pre-clap CLI subcommand that
//! `bin/qd/main.rs` dispatches AND a /proc identity marker, so the CLI's verb table now
//! names a qw constant. That is legal — the dependency edge is qd → qw, never the
//! reverse — and pi does exactly this with its own `pi-daemon` verb
//! ([`super::super::pi::residence::PI_ADAPTER_VERB`]).
//!
...
//!   and serves the ws front ([`super::serve`]) until SIGTERM. It outlives
...
pub fn run_adapter(args: &[String]) -> i32 {
    ...
    eprintln!(
        "acp-daemon: ready — listening {}, session {session}",
        opts.listen
    );
...
mod tests {
    ...
    fn parse_adapter_args_required_and_forms() {
        ...
        let l2 = parse_adapter_args(&[
            "--listen=ws://x:1".into(),
            "--cwd=.".into(),
            "--load-session=s2".into(),
        ])
        .unwrap();
    ...
    fn cmdline_identity_requires_marker_and_endpoint() {
        ...
        assert!(cmdline_is_our_acp_daemon(
            Some("qd acp-daemon --listen x"),
            None
        ));
    }

    /// Ruling D6 put the `acp-daemon` verb on the `qw` binary, so residents are
    /// spawned as `qw acp-daemon` from now on — while residents ALREADY RUNNING when
    /// it landed were spawned as `qd acp-daemon`. This probe keys on
    /// [`ADAPTER_VERB`], never on the program, so BOTH must latch: one that stopped
    /// recognising the old spelling would orphan every live resident on upgrade, and
    /// one that did not recognise the new spelling would orphan every resident
    /// created after it.
    #[test]
    fn cmdline_identity_is_program_agnostic_across_the_qd_qw_move() {
        let ep = "ws://127.0.0.1:9000";
        for exe in ["/usr/local/bin/qd", "/usr/local/bin/qw"] {
            let cmd = format!("{exe} acp-daemon --listen {ep} --cwd /w");
            assert!(
                cmdline_is_our_acp_daemon(Some(&cmd), Some(ep)),
                "the {exe} spelling must latch: {cmd}"
            );
            assert!(cmdline_is_our_acp_daemon(Some(&cmd), None));
        }
    }
}
