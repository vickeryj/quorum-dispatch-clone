...
mod tests {
    ...
    fn r8_terminal_reason_mapping_is_explicit() {
        ...
        assert_eq!(
            StopReason::EndTurn.to_terminal_reason(),
            TerminalReason::Completed
        );
        assert_eq!(
            StopReason::Cancelled.to_terminal_reason(),
            TerminalReason::Cancelled
        );
        assert_eq!(
            StopReason::MaxTurnRequests.to_terminal_reason(),
            TerminalReason::MaxTokens
        );
        assert_eq!(
            StopReason::MaxTokens.to_terminal_reason(),
            TerminalReason::MaxTokens
        );
        // Forward-compat only — CC never emits this; do NOT gate CC refusal handling on it.
        assert_eq!(
            StopReason::Refusal.to_terminal_reason(),
