...
// HARNESS-FIRST. One directory per agent program, each grouping its own code by
// the transport that carries messages to it (`pty/`, `relay/`, `app_server/`,
// `stdio/`, `daemon/`). Transport machinery that MORE THAN ONE harness runs —
// the ACP spine above all — lives in `shared/` instead of being duplicated or
// filed under a harness it does not belong to. See `shared/mod.rs` for the test
// an item must pass to live there.
pub mod claude;
pub mod codex;
pub mod opencode;
pub mod pi;
...
// Compatibility re-exports: `provider::acp`, `provider::pane` and
// `provider::fixture` were top-level modules before the harness-first reorg, and
// `dispatch` reaches them by those paths from ~200 call sites. Re-exporting the
// modules keeps every one of those paths resolving, so the move stays a
// relocation rather than a workspace-wide rewrite.
pub use shared::{acp, fixture, pane};

pub use shared::fixture::FixtureDaemonProvider;
...
pub enum Hosting {
    ...
    Daemon,
    /// Session is a TUI in a mux pane that ALSO serves a control channel —
    /// `pi/extension`. Pane-shaped for every lifecycle purpose; distinct because
    /// the row must be able to SAY so.
    ///
    /// This variant is what keeps a pi/extension row from degrading. Without it
    /// `parse_hosting("extension")` answers `None`, the caller falls back to pi's
    /// structural `Daemon`, and a pane-hosted session is addressed as a resident
    /// that was never spawned — the exact mis-routing `row_hosting` exists to
    /// prevent, arriving through the door left open by a token nothing parses.
    Extension,
    /// A daemon a human can ALSO open a terminal onto: codex's
    /// `codex app-server --listen ws://…`, joinable by a `codex --remote` TUI as
...
///
/// Step 2 is what keeps every pre-existing row byte-stable: only the
/// codex-interactive create path and the `codex/app-server` create path write the
/// field, so every claude/acp/pi/codex-daemon row answers exactly as it did
/// before this seam existed. In particular `row_hosting("codex", None)` is still
/// [`Hosting::Daemon`] — a codex row that does not say is the plain app-server
...
pub struct LaunchRequest {
    ...
    /// `pi/extension`: the unix socket the in-process `quorum-lane` extension
    /// should serve, emitted as `--quorum-sock <path>`.
    ///
    /// Provider-neutral in the same sense `interactive` is — it states what the
    /// caller wants, and each impl answers for itself. Only pi's `launch_plan`
    /// reads it; every other provider ignores it, because no other harness in
    /// this repo has an in-process extension surface to serve one
    /// (`Harness::supports` records that as structural).
    ///
    /// `None` on every other lane, which is why the `Default` derive keeps all
...
mod tests {
    ...
    fn row_hosting_lets_the_row_override_the_provider() {
        ...
        assert_eq!(
            row_hosting("codex", Some("app-server")),
            Some(Hosting::AppServer)
        );
    }
    ...
    fn hosting_tokens_round_trip_through_parse() {
        ...
        for h in [Hosting::MuxPane, Hosting::Daemon, Hosting::Extension, Hosting::AppServer] {
            assert_eq!(parse_hosting(h.as_str()), Some(h));
        }
        assert_eq!(parse_hosting("nonsense"), None);
    }

    /// `Hosting` and `crate::lane::Mode` are two spellings of one fact, in two
    /// layers that cannot name each other (a dependency cycle). Nothing but this
    /// test stops them drifting — and a drift is not cosmetic: a token one side
    /// writes and the other cannot parse degrades the row to the harness default,
    /// which routes a pane-hosted session into a daemon carrier.
    #[test]
    fn the_hosting_and_mode_vocabularies_are_the_same_tokens() {
        use crate::lane::Mode;
        for (h, m) in [
            (Hosting::MuxPane, Mode::Pane),
            (Hosting::Daemon, Mode::Daemon),
            (Hosting::Extension, Mode::Extension),
        ] {
            assert_eq!(h.as_str(), m.hosting_token(), "{h:?} vs {m:?}");
            assert_eq!(Mode::from_hosting_token(h.as_str()), Some(m));
            assert_eq!(parse_hosting(m.hosting_token()), Some(h));
        }
    }
