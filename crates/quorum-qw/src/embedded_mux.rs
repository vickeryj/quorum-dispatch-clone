...
/// real `qd` or `qw` binary). Read ONLY here; never consulted on a non-test path by
/// value.
const DAEMON_PROGRAM_ENV: &str = "QD_EMBEDDED_DAEMON_PROGRAM";

/// The argv-prefix the embedder uses for its hidden daemon entry. Bound to
/// [`crate::qrmux_server::VERB`] rather than spelled a second time, because the
/// launcher and the binary that ANSWERS it must never drift: they did drift once —
/// ruling D6 — when `current_exe()` became `qw` and only `qd` carried the verb.
const DAEMON_ARGS_PREFIX: &str = crate::qrmux_server::VERB;

/// Build the embedder's [`ServerLaunchSpec`]: re-exec `current_exe()` with
/// `["qrmux-server"]`, so the daemon cold-start runs `<exe> qrmux-server
/// --socket-dir <dir>` — NOT `current_exe() server` (the bug: neither binary has a
/// bare `server` verb). `current_exe()` is `qw` whenever a lane call is being served
/// out of the qw subprocess and `qd` when a qd verb embeds the mux directly; BOTH
/// dispatch `qrmux-server`, which is what makes re-execing self safe here. The
/// program is overridable by [`DAEMON_PROGRAM_ENV`] for the mutation-control test
/// ONLY.
...
pub fn embedded_pending_delivery(
    socket_dir: &Path,
    args: PendingDeliveryArgs,
) -> io::Result<String> {
    ...
    match out {
        ...
        Ok(Err(e)) => Err(io::Error::other(format!(
            "embedded mux: pending_delivery: {e}"
        ))),
