...
// `Serialize`/`Deserialize`/`Eq` are here because this phase CROSSES THE qd/qw
// BOUNDARY: `LaneError::StartFailed` carries it so `qd start -p`'s
// `priming-readiness-timeout` record can name the phase TYPED, exactly as the
// in-process caller did. Re-deriving it by string-matching the detail wording is
// the m-4 coupling this type was introduced to delete, and a process boundary
