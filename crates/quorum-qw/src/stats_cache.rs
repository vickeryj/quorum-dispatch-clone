...
//! ([`crate::gather::gather`] is exhaustive; the `-n` cap is applied later in the pure
...
//! the `read_stats` call sites inside `provider_gather`'s per-provider steps.
...
impl StatsCache {
        ...
        F: FnOnce(&Path) -> (JsonlStats, Option<SessionStatus>),
    {
        ...
        if let Some(entry) = self.entries.get(path) {
            if entry.mtime_ms == mtime_ms && entry.size == size {
                if let (Some(status), Some(full)) = (entry.status.servable(), entry.stats.to_full())
                {
                    self.hits += 1;
                    self.touched.insert(path.to_path_buf());
...
mod tests {
    ...
    fn warm_status_read_is_counted_then_served_warm() {
        ...
        for p in &f.files {
            let (_stats, status) =
                warm.get_or_read_with_status(p, false, |pp| warm_reader.read(pp));
            assert_eq!(
                status,
        ...
        assert_eq!(
            ...
            0,
            "cache saw ZERO reads — stats AND status"
        );
    ...
    fn memoized_none_status_served_warm_as_none() {
        ...
        assert_eq!(
            status, None,
            "memoized derived-None served warm, not re-derived to Busy"
        );
        assert_eq!(
            warm_reader.count(),
            0,
            "a derived-None is a real HIT, not a miss"
        );
    ...
    fn stats_only_entry_forces_a_status_rederive() {
        ...
        assert_eq!(
            status,
            Some(SessionStatus::Busy),
            "re-derived, not a phantom Idle/None"
        );
        assert_eq!(
            status_reader.count(),
            1,
            "an Absent-status entry forces a status re-derive"
        );
        assert_eq!(warm.transcript_reads(), 1, "the re-derive IS counted");
        assert_eq!(
            warm.hits(),
            0,
            "an Absent-status entry is a status MISS, not a hit"
        );
        warm.persist().unwrap();
        ...
        assert_eq!(
            status2,
            Some(SessionStatus::Busy),
            "upgraded entry serves the derived status warm"
        );
    ...
    fn pre_fix_v1_snapshot_rebuilds_never_phantom_status() {
        ...
        assert!(
            cache.loaded_empty(),
            "a v1 snapshot rebuilds wholesale (schema bump)"
        );
        ...
        assert_eq!(
            status,
            Some(SessionStatus::Busy),
            "freshly derived, never a phantom"
        );
        assert_eq!(
            reader.count(),
            1,
            "the rebuild re-read (the v1 entry never served)"
        );
    }
    ...
    fn preview_then_no_preview_and_vice_versa() {
        // Order A: cold WITHOUT previews, warm WITH previews.
        {
            ...
            assert_eq!(
                reader.count(),
                reads_before,
                "served previews with ZERO reads"
            );
        }
        // Order B: cold WITH previews, warm WITHOUT previews.
        {
            ...
            for p in &f.files {
                ...
                assert!(
                    s.last_turns.is_none(),
                    "no-preview request → previews nulled"
                );
    ...
    fn old_schema_snapshot_rebuilds() {
        ...
        for p in &f.files {
            ...
            assert_eq!(
                s.turns, 1,
                "correct fresh rows, not the stale schema-9999 turns=7"
            );
        }
    ...
    fn delete_never_errors() {
        ...
        assert_eq!(
            gone,
            JsonlStats::default(),
            "deleted → zeroed default, no error"
        );
    ...
    fn pure_hit_run_is_not_dirty() {
        ...
        assert!(
            !warm.dirty,
            "an all-hit run stays clean → persist is a no-op"
        );
    }
}
