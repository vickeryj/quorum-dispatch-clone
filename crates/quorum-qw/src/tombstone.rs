...
//!
//! LIVES IN qw, at the TOP LEVEL beside [`crate::registry`] (qd/qw split): the code
//! here is provider-agnostic — plain JSON over a path — but the CONTRACT is not, and
//! the contract is what places it. Read the **Scope** note above: this store is
//! written ONLY for provider `acp/claude-code`, and its only writers are the ACP
//! transport-loss paths. That makes it provider-specific state, and ALL
//! provider-specific code belongs in `quorum-qw`. It sits at the top level rather
//! than under `provider/acp/` because it is a STORE, and its sibling store
//! `registry.rs` — the janitored claude row this one exists to outlive — is already
//! here; the two are read together or not at all. It was `dispatch::tombstone` and
//! moved as a pure relocation; `dispatch` re-exports it under the old flat name so
//! `dispatch::tombstone::…` (the `acp_loss` verb + the `acp_fallback` lane) keeps
//! resolving.
...
mod tests {
    ...
    fn corrupt_record_reads_none_and_is_overwritable() {
        ...
        assert!(
            read_tombstone(state_dir, "sess-x").is_none(),
            "corrupt ⇒ None, never an error"
        );
