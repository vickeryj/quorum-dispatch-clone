...
///
/// `verb` is the CALLER's command word, stamped into the one line this loop
/// writes (the degradation warning). It is a parameter for the reason
/// [`crate::delivery::CarrierError::line`] takes one: this body is a library
/// loop, not a verb, so it cannot know which command the user typed. It had
/// `qd wait: ` hardcoded, which was correct only because `crate::idle`'s claude
/// arm is its single caller — a second caller would have printed the wrong
/// command's name with nothing to catch it.
    ...
    while deps.now_ms() - start < timeout_ms {
        ...
        if content_alive && !saw_turn_end {
            match deps.poll_turn_end() {
                ...
                Err(why) => {
                    deps.warn(&format!(
                        "qd {verb}: transcript unavailable mid-wait ({why}) — continuing status-keyed only"
                    ));
                    content_alive = false;
                }
            }
...
mod tests {
    ...
    fn busy_then_idle_is_done() {
        ...
        assert_eq!(
            run_wait_content_loop(&f, "wait", 60_000, 500),
    ...
    fn idle_without_evidence_keeps_polling_to_timeout() {
        ...
        assert_eq!(
            run_wait_content_loop(&f, "wait", 1500, 500),
            WaitStatusOutcome::Timeout
        );
    }
    ...
    fn degrade_on_source_error_warns_once_and_stays_status_keyed() {
        ...
        // The BYTES, pinned: the warning names the command that is waiting. The
        // loop used to hardcode `qd wait: ` here; it now stamps the caller's
        // verb, and this row proves the caller's verb is what comes out.
        assert_eq!(
            f.warns.borrow()[0],
            "qd wait: transcript unavailable mid-wait (transcript shrank below the entry offset (10 < 20 bytes)) — continuing status-keyed only",
            "the degradation warning is `qd <verb>:`-attributed"
        );
    }

    #[test]
    fn degradation_warn_names_the_callers_verb_not_a_hardcoded_one() {
        // SHAPE ROW (the teeth on the `verb` parameter): a caller that is not
        // `qd wait` must see ITS OWN command on the line. With the pre-fix
        // hardcode this row prints `qd wait:` and reds — which is exactly the
        // bug the parameter exists to make impossible.
        let f = FakeWait::new(|_| Some("idle"), |_| Err("shrank".into()));
        assert_eq!(
            run_wait_content_loop(&f, "send:pty --wait", 1500, 500),
            WaitStatusOutcome::Timeout
        );
        assert_eq!(
            f.warns.borrow()[0],
            "qd send:pty --wait: transcript unavailable mid-wait (shrank) — continuing status-keyed only"
        );
    }
    ...
    fn b5ii_iii_daemon_offline_is_session_exited_not_stale_busy() {
        ...
        assert_eq!(
            run_wait_content_loop(&deps, "wait", 60_000, 0),
            WaitStatusOutcome::SessionExited,
    ...
    fn b5ii_iii_dead_daemon_stale_busy_times_out_not_hangs() {
        ...
        assert_eq!(
            run_wait_content_loop(&deps, "wait", 3_000, 0),
            WaitStatusOutcome::Timeout,
    ...
    fn real_barrier_closes_w6_window_end_to_end() {
        ...
        assert_eq!(
            run_wait_content_loop(&deps, "wait", 60_000, 500),
            WaitStatusOutcome::Done,
    ...
    fn real_barrier_removed_w6_window_times_out() {
        ...
        assert_eq!(
            run_wait_content_loop(&deps, "wait", 1500, 0),
            WaitStatusOutcome::Timeout,
    ...
    fn channel_outcomes_match_today_on_equivalent_inputs() {
        ...
        assert_eq!(
            run_wait_content_loop(&d1, "wait", 60_000, 0),
            WaitStatusOutcome::Done
        );
        ...
        assert_eq!(
            run_wait_content_loop(&d2, "wait", 60_000, 0),
            WaitStatusOutcome::SessionExited
        );
        ...
        assert_eq!(
            run_wait_content_loop(&d3, "wait", 1500, 0),
            WaitStatusOutcome::Timeout
        );
        ...
        assert_eq!(
            run_wait_content_loop(&d4, "wait", 1500, 0),
            WaitStatusOutcome::Timeout
        );
    }
