...
/// The directory holding **qd's** intent log — the other half of the two-log
/// ledger (`doc/tbd/provider-architecture/09-ledger-split.md`).
///
/// # Why there are two, and why the split is by OWNER rather than by kind
///
/// The ruling is "two logs, and qd never reads qw's." qd's holds the send
/// **intent**; qw's holds **delivery and terminal**. They are separate files
/// because a shared one is a wire protocol with none of the properties you want
/// from one — unversioned, unenumerable, and impossible to know who depends on
/// which field.
///
/// The dead-writer rule is what makes the split load-bearing rather than tidy.
/// It has three clauses and only one crosses:
///
/// - (a) no terminal for this `send_id` — needs **qw's** log
/// - (b) the writer *incarnation* is gone — pid dead, or a recycled pid held by
///   a stranger — needs the intent record's own `pid`/`start_ms`
/// - (c) age > [`T_ANCHOR_IDLE_MS`] — needs that record's `ts`
///
/// Clauses (b) and (c) are about **qd's own record and qd's own process**, so
/// they stay in qd, and qd needs a record of its own to read them from. That is
/// what this file is. Before the split qd read them out of qw's file, which is
/// exactly the unbounded coupling the ruling removes.
///
/// Named for its CONTENT, not its owner: a path called `qd/` would have to be
/// renamed the day a record changes hands, and paths that encode ownership rot
/// silently because nothing fails when the owner moves.
pub fn intent_dir(state_dir: &Path) -> PathBuf {
    state_dir.join("intent")
}

/// qd's intent log for one session key. The qd-side twin of [`events_path`].
pub fn intent_path(state_dir: &Path, key: &str) -> PathBuf {
    intent_dir(state_dir).join(format!("{key}.events.jsonl"))
}
...
/// The two files an INTENT reader must merge, the qd-side twin of
/// [`reader_paths`].
///
/// Both keys for the same reason the delivery side has both: a send made before
/// its session id resolved is keyed `byname-<name>`, and a reader given only one
/// of the two would miss half a session's intents.
pub fn intent_reader_paths(
    state_dir: &Path,
    session_id: Option<&str>,
    name: Option<&str>,
) -> Vec<PathBuf> {
    let mut out = Vec::new();
    if let Some(sid) = session_id.filter(|s| !s.is_empty()) {
        out.push(intent_path(state_dir, sid));
    }
    if let Some(n) = name.filter(|n| !n.is_empty()) {
        out.push(intent_path(state_dir, &byname_key(n)));
    }
    out
}

/// Read + merge **qd's intent log**, the twin of [`read_merged`].
pub fn read_intent_merged(
    state_dir: &Path,
    session_id: Option<&str>,
    name: Option<&str>,
) -> ReadResult {
    let mut merged = ReadResult::default();
    for path in intent_reader_paths(state_dir, session_id, name) {
...
impl<'a> ReaderCtx<'a> {
    /// Read + merge this context's **delivery** event file(s) (§4.1 / §4.4) —
    /// qw's half of the ledger.
    pub fn read(&self) -> ReadResult {
        read_merged(self.state_dir, self.session_id, self.name)
    }

    /// Read + merge this context's **intent** file(s) — qd's half.
    ///
    /// The two never merge into one `ReadResult`. Folding both sides on a
    /// correlation id is the tempting shortcut and it is the same unbounded
    /// coupling as reading an address file, applied to a bigger schema: it would
    /// make qw's on-disk layout a second, unversioned contract that qd depends
    /// on. Anything qd needs from the far side crosses as
    /// [`crate::contract::LaneOps::resolved`],
    /// [`crate::contract::LaneOps::await_terminal`] or
    /// [`crate::contract::LaneOps::recover`].
    pub fn read_intent(&self) -> ReadResult {
        read_intent_merged(self.state_dir, self.session_id, self.name)
    }
}
...
impl EventWriter {
    ...
    /// Bind a writer for `key` in **qd's intent log** ([`intent_path`]).
    ///
    /// The twin of [`EventWriter::for_key`], and deliberately a separate
    /// constructor rather than a `which_log: enum` parameter: every call site
    /// then states which side of the ledger it is writing to, in a form a source
    /// scan can check. A parameter would put that decision in a variable, where
    ...
    pub fn for_intent(
        ...
        Self::new(intent_path(state_dir, key), session, name)
    }
...
pub enum LadderEvent {
    /// A rung was entered (`rung` = the `recovery::Rung::as_str` (dispatch-side) token).
...
pub fn is_dead_dangling(records: &[EventRecord], si: &EventRecord, now_ms: i64) -> bool {
    ...
    // (b) + (c), which are about the RECORD and its WRITER, never the record set.
    writer_gone_and_stale(si, now_ms)
}

/// Clauses **(b) and (c) alone** of the dead-writer rule (§7) — the half that
/// reads nothing but `si`'s own envelope.
///
/// This exists because the ledger is two logs
/// (`doc/tbd/provider-architecture/09-ledger-split.md`). Clause (a) — "is there
/// a terminal for this send_id" — is answerable only from **qw's** delivery log,
/// and qd does not read it; clauses (b) "the writer incarnation is gone" and (c)
/// "age > [`T_ANCHOR_IDLE_MS`]" read the `send-initiated` record's own
/// `pid`/`start_ms`/`ts`, which qd wrote and qd owns. `qd delivery:recover`
/// evaluates THIS over its own intent record and crosses the boundary for (a).
///
/// Split out rather than reimplemented on the qd side on purpose: the RF-6
/// start_ms arm is the subtle part of the fence, and a second copy of it is a
/// second thing to get wrong. [`is_dead_dangling`] is now literally (a) plus this,
/// so the two callers cannot drift.
pub fn writer_gone_and_stale(si: &EventRecord, now_ms: i64) -> bool {
...
pub enum Received {
    ...
    /// §X.3.5 — the `seen-failed` terminal: delivery reached the wire and the
    /// on-received phase then failed (the mux's `recipient-gone`, and the relay
    /// server's twin). A FAILURE, and deliberately not folded into
    /// [`Received::SendFailed`]: that one is the pre-wire DOOR refusal, this one
    /// is a post-wire non-delivery, and a reader that cannot tell them apart
    /// cannot tell the operator which half to look at. `reason` carries the
    /// ledger's own token.
    SeenFailed {
        reason: String,
    },
...
pub(crate) fn received_from_terminal(rec: &EventRecord) -> Received {
    match rec.event.as_str() {
        ...
        // §X.3.4/§X.3.5 — the 3-phase ON-RECEIVED terminals, and the two the
        // 7-set has carried since the polite-delivery lane landed. They were
        // missing here, so both fell to the catch-all below and came back as
        // `BudgetExhausted{"unknown-terminal"}` — which `terminal_from_received`
        // turns into `Terminal::Undetermined`. `message-seen` is THE success
        // terminal (`is_success_terminal` is its one home), so that mapping
        // reported a CONFIRMED DELIVERY as "I could not tell", and `seen-failed`
        // reported a confirmed non-delivery the same way. Latent only because
        // `await_received`'s sole caller (`LaneOps::await_terminal`) had no
        // production caller of its own; the moment `qd send:pty --wait` became
        // one, every embedded delivery resolved through here.
        //
        // `message-seen` maps onto `Anchored` rather than growing a variant
        // because `Received`'s subject is the OUTCOME ("the send reached the
        // recipient"), not which event spelled it — the two success terminals
        // are one outcome, which is exactly what `is_success_terminal` says.
        // `seen-failed` does NOT map onto `SendFailed`: see [`Received::SeenFailed`].
        "message-seen" => Received::Anchored,
        "seen-failed" => Received::SeenFailed {
            reason: rec.str_field("reason").unwrap_or_default(),
        },
...
mod tests {
    ...
    fn r3d_recovery_episode_reconstructs_from_log_alone() {
        ...
        let episode = vec![
            LadderEvent::RungEntered {
                session_id: sid.into(),
                rung: "pidfd-signal".into(),
            },
            LadderEvent::RungTimeout {
                session_id: sid.into(),
                rung: "pidfd-signal".into(),
    ...
    fn d1_send_failed_is_terminal_and_queued_kinds_are_not() {
        ...
        for non in [
            ...
            assert!(
                !is_terminal(non),
                "{non} must stay NON-terminal (queued is honest)"
            );
        }
    }
