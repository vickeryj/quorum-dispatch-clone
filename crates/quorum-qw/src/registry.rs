...
pub fn tombstone_guarded(
    ...
    if let (Some(expected), Some(entry)) = (expected_started_at, read_entry(sessions_dir, pid)) {
...
mod tests {
    ...
    fn ensure_tombstone_guarded_none_expected_synthesizes_like_unguarded() {
        ...
        let back: RegistryEntry = serde_json::from_slice(&std::fs::read(&tomb).unwrap()).unwrap();
    ...
    fn read_tombstoned_entry_reads_the_renamed_file() {
        ...
        assert!(
            ...
            "tombstone must find the live row"
        );
        ...
        let tombstoned =
            read_tombstoned_entry(dir.path(), 4242).expect("the tombstoned file must be readable");
    ...
    fn claim_incarnation_reads_absent_as_zero_and_roundtrips() {
        ...
        assert_eq!(
            claim_incarnation(legacy.as_bytes()),
            0,
            "legacy claim => incarnation 0"
        );
        // Unparseable blob => 0 (serde(default) semantics).
        assert_eq!(
            claim_incarnation(b"not json"),
            0,
            "corrupt claim => incarnation 0"
        );
    ...
    fn claim_name_with_incarnation_bumps_monotonically_inside_the_section() {
        ...
        assert_eq!(
            claim_incarnation(&body1),
            1,
            "incarnation 1 stamped in the file"
        );
        ...
        assert_eq!(
            claim_incarnation(&body2),
            2,
            "incarnation 2 stamped after reap"
        );
        ...
        assert_eq!(
            claim_incarnation(&body3),
            2,
            "a refused reclaim leaves the fence at 2"
        );
    }
