...
async fn main() -> anyhow::Result<()> {
    ...
    let before = qrmux::client::session_client::get_history_session(Some(dir), &name).await?;
    // wheel-up x5 (SGR encoding, button 64) at col 40, row 10
    let wheel: Vec<u8> = b"\x1b[<64;40;10M".repeat(5);
    qrmux::client::session_client::send_input_session(Some(dir), None, &name, wheel).await?;
    tokio::time::sleep(std::time::Duration::from_millis(800)).await;
    let after = qrmux::client::session_client::get_history_session(Some(dir), &name).await?;
    ...
    qrmux::client::session_client::send_input_session(Some(dir), None, &name, wheeldown).await?;
    Ok(())
}
