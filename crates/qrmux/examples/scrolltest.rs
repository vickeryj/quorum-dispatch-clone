...
async fn main() -> anyhow::Result<()> {
    ...
    let dir_s = "/tmp/qrmux-probe".to_string();
    ...
    let lines = qrmux::client::session_client::get_history_session(Some(dir), name).await?;
    ...
    qrmux::client::session_client::kill_session_session(Some(dir), name).await?;
    eprintln!("killed");
    Ok(())
}
