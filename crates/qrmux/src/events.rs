...
/// Default per-file size cap (5 MiB). Overridable via `QRMUX_EVENTS_MAX_BYTES`
...
pub enum DaemonEvent {
    ...
    /// Optional liveness tick, DEFAULT OFF (QRMUX_EVENTS_HEARTBEAT_MS).
    Heartbeat {
        #[serde(flatten)]
        meta: EventMeta,
    },
...
pub struct EventsCtx {
    ...
    pub dir: PathBuf,
    /// Per-file cap (QRMUX_EVENTS_MAX_BYTES, default 5 MiB).
    pub max_bytes: u64,
}
...
impl EventsCtx {
    ...
    ///
    /// `QRMUX_EVENTS_DISABLED=1` → `None` (loud single warn). This kill switch
    ...
    pub fn from_env(socket_dir: &Path) -> Option<Self> {
        if std::env::var("QRMUX_EVENTS_DISABLED")
            .map(|v| v == "1")
            .unwrap_or(false)
        {
            tracing::warn!("QRMUX_EVENTS_DISABLED=1 — daemon event stream is OFF (advisory write-log will not be produced)");
            return None;
        }
        let max_bytes = std::env::var("QRMUX_EVENTS_MAX_BYTES")
            .ok()
...
impl EventWriter {
    ...
    /// Optional heartbeat tick (QRMUX_EVENTS_HEARTBEAT_MS). No-op after close
