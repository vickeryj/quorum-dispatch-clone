...
/// `qrmux.sock` (10 bytes); we round up to leave headroom for `qrmux.lock`,
...
// ===========================================================================
// Override seam (C1 D1): the engine resolves the qrmux dir itself and passes it
...
// `lock_path[_for]`, which computed `<dir>/qrmux.sock` / `<dir>/qrmux.lock`) are
// RETIRED (spec §1, §9). Per-session callers use `session_socket_path_for` /
// `session_lock_path_for`. The ONLY remaining reference to the `qrmux.sock` NAME
// is `discovery::LEGACY_LEAF` — the exclusion-guard const in `scan_sessions` that
// prevents the shared-socket leaf from being probed as a session name. Nothing
// binds or expects the shared leaf anymore; the §5.3 legacy-visibility probe
// was dropped in the cat-(iii) clean break.
...
// `<dir>/qrmux.sock`. The dir resolution is UNCHANGED (no third scheme,
...
/// W-1 named divergence). `qrmux` would collide with the legacy `qrmux.sock` leaf;
...
const RESERVED_SESSION_NAME: &str = "qrmux";
...
/// `QRMUX_TEST_SHARED=1`, the per-session daemon split is DELIBERATELY collapsed
...
pub fn shared_fate_test_mode() -> bool {
    std::env::var("QRMUX_TEST_SHARED").ok().as_deref() == Some("1")
}
...
/// 2. REFUSE the reserved `qrmux` leaf (legacy-socket collision), and
...
mod tests {
    ...
    fn socket_dir_sun_path_guard_trips_on_overlong_root() {
        // A home root long enough that <root>/.quorum/dispatch/mux + qrmux.sock > 104 bytes.
    ...
    fn socket_dir_rejects_symlink() {
        ...
        // Renamed retach → qrmux with the socket-layout port (spec ground rule 3).
        let sym_dir = tmp.path().join("qrmux");
    ...
    /// `--session` identity validation: reserved `qrmux` and leading-`.` are
    ...
    fn validate_session_identity_tightening() {
        ...
        let err = validate_session_identity("qrmux").unwrap_err().to_string();
