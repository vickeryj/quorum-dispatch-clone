# qrmux Integration Test Suite

Port of B1's 6-point harness scenarios (G1–G6) into Rust integration tests, plus
negative control scaffold. Stress validates the qrmux daemon under stress conditions
matching the B1 fork-decision baseline.

## The 6-Point Gate

Each scenario is a proof checkpoint for qrmux under production-like conditions:
...
**Negative control:** `QRMUX_TEST_BREAK=drop1000` breaker (drops every 1000th PTY byte)
...
```bash
cd crates/qrmux
cargo test -p qrmux --test integration_tests -- --nocapture
```

### Individual scenario

```bash
cargo test -p qrmux --test integration_tests g1_send_verb -- --nocapture
cargo test -p qrmux --test integration_tests g2_altscreen_stress -- --nocapture
```

### Negative control (M3, currently ignored)

```bash
cargo test -p qrmux --test integration_tests negative_control_breaker -- --ignored --nocapture
```

### Unit tests within qrmux (M1 baseline: 778 passing)

```bash
cargo test -p qrmux --lib
```
...
qrmux is a **screen-model mux** (not a passthrough). The vte performer consumes
...
Evidence: bare PTY with NO mux echoed 246/2000 tokens at 72KB burst; qrmux ingest
instrumented lossless every run; app-output complete (2000/2000). The mux cannot
replay bytes the kernel never delivered.

**Consequence:** All assertions on echo data in qrmux must be keyed on
