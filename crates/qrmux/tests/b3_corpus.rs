...
//! corpus dir is provided via `QRMUX_CORPUS_DIR`, the rows do NOT silently pass
...
//!      what is missing, REGARDLESS of whether `QRMUX_CORPUS_DIR` is set. They do
...
//!   1. `QRMUX_CORPUS_DIR` UNSET (active rows) → CORPUS-PENDING. eprintln +
//!      evidence file + early return. Test PASSES (gate stays green), records
//!      PENDING, never PASS.
//!   2. `QRMUX_CORPUS_DIR` SET, fixture file ABSENT (active rows) → test FAILS
//!      loudly (`CORPUS-PENDING-BUT-REQUESTED`): the operator asked for corpus
//!      judging but the required fixture is not there.
//!   3. `QRMUX_CORPUS_DIR` SET, fixture PRESENT → NON-DEGENERACY gate (min line
...
//! ============================================================================
//! `QRMUX_CORPUS_DIR` points at the golden fixtures root (the repo's
...
struct RowSpec {
    ...
    row: &'static str,
    /// Subdirectory under `$QRMUX_CORPUS_DIR` holding this row's fixture
    ...
    /// `QRMUX_CORPUS_DIR` does not flip it to FAIL, and it is never faked against
...
fn run_corpus_row(spec: &RowSpec) -> Result<(), String> {
    ...
    // be faked against an unrelated fixture, NOR fail when QRMUX_CORPUS_DIR is
    ...
    let corpus_dir = std::env::var("QRMUX_CORPUS_DIR")
        .ok()
        .filter(|s| !s.is_empty());

    // ---- Mode 1: QRMUX_CORPUS_DIR UNSET → CORPUS-PENDING (loud skip) --------
    let Some(corpus_dir) = corpus_dir else {
        let verdict = format!("CORPUS-PENDING (QRMUX_CORPUS_DIR unset; pin {CORPUS_PIN})");
        eprintln!(
            "\n\
             ============================================================\n\
             [{row}] {verdict}\n\
             comparator={comp:?}  expected_subdir={subdir}/{file}\n\
             This row is SKIPPED (not PASS): set QRMUX_CORPUS_DIR to the golden\n\
    ...
    if !fixture_path.exists() {
        let verdict = format!(
            "CORPUS-PENDING-BUT-REQUESTED (QRMUX_CORPUS_DIR set to {dir}, but fixture {path} \
             is MISSING; pin {pin})",
...
// without needing a real corpus). These do NOT touch QRMUX_CORPUS_DIR-driven
...
mod skeleton_selftest {
    ...
    fn pending_by_design_returns_ok_and_records_reason() {
        ...
        // `QRMUX_CORPUS_DIR` read — so it cannot be flipped to FAIL by a set dir,
    ...
    fn missing_fixture_when_dir_set_is_error() {
        ...
        let tmp =
            std::env::temp_dir().join(format!("qrmux-corpus-selftest-{}", std::process::id()));
