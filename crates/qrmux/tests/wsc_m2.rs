...
//!
//! These drive a REAL `qrmux server --session <name> --socket-dir <dir>` child
...
/// in the jail with the given extra env (e.g. `QRMUX_CLAIM_TIMEOUT_MS`). Daemon
...
/// legacy `qrmux.sock`), and the daemon accepts its own-name create + list.
#[test]
fn single_session_binds_name_dot_sock() -> Result<(), Box<dyn Error>> {
    ...
    assert!(
        !jail.socket_dir.join("qrmux.sock").exists(),
        "single-session mode must NOT bind the legacy qrmux.sock"
    );
...
fn claim_timeout_unclaimed_daemon_exits() -> Result<(), Box<dyn Error>> {
    ...
    let mut daemon = start_single_session_daemon(
        &jail,
        "ghost",
        &[("QRMUX_CLAIM_TIMEOUT_MS".into(), "800".into())],
        Some(&stderr),
        true,
    )?;
    let socket = daemon.socket.clone();
...
fn claim_timer_not_reset_by_hello_only_probes() -> Result<(), Box<dyn Error>> {
    ...
    let mut daemon = start_single_session_daemon(
        &jail,
        "ghost",
        &[("QRMUX_CLAIM_TIMEOUT_MS".into(), "1000".into())],
        None,
        true,
    )?;
    let socket = daemon.socket.clone();
...
fn claim_cancelled_by_session_create() -> Result<(), Box<dyn Error>> {
    ...
    let mut daemon = start_single_session_daemon(
        &jail,
        "alpha",
        &[("QRMUX_CLAIM_TIMEOUT_MS".into(), "600".into())],
        None,
        true,
    )?;
    let socket = daemon.socket.clone();
    ...
    let killed = std::thread::spawn({
        ...
        move || -> Result<(), String> {
            ...
            rt.block_on(async {
                ...
                use qrmux::protocol::{ClientMsg, ServerMsg};
...
/// `--session`, bound `qrmux.sock`) is RETIRED — `--session` is now REQUIRED on
/// `qrmux server`. Spawning the daemon WITHOUT `--session` must FAIL LOUDLY
/// (clap rejects the missing required arg, exit code != 0, named "session" on
/// stderr) and must NOT bind any `qrmux.sock`. This is the retire-with-reason
...
fn server_without_session_is_rejected_loudly() -> Result<(), Box<dyn Error>> {
    ...
    // Spawn `qrmux server --socket-dir <dir>` with NO --session and capture stderr.
    let out = Command::new(qrmux_binary())
        .arg("server")
        .arg("--socket-dir")
        .arg(&jail.socket_dir)
        .env_clear()
    ...
    assert!(
        !out.status.success(),
        "qrmux server WITHOUT --session must fail (legacy shared mode retired)"
    );
    ...
    assert!(
        !jail.socket_dir.join("qrmux.sock").exists(),
        "a rejected (no --session) daemon must NOT bind the legacy qrmux.sock"
    );
