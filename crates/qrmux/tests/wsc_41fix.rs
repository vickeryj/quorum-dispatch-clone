...
//!
//! These arms drive a REAL single-session `qrmux server --session <name>
...
//! seam (`QRMUX_TEST_SLOW_DROP_MS`, breaker pattern — documented, never set by
...
//!     QRMUX_EXIT_WAIT_MS small (500) → the daemon exits within bound+slack with
...
use libmod::client::{qrmux_binary, sweep_orphan_daemons, ProtocolClient};
...
fn arm1_kill_reply_survives_slow_drop() -> Result<(), Box<dyn Error>> {
    ...
    let mut daemon = start_daemon(
        ...
        &[
            ...
            ("QRMUX_TEST_SLOW_DROP_MS".into(), "400".into()),
        ],
        true,
    )?;
...
/// QRMUX_EXIT_WAIT_MS (500ms). The daemon must EXIT within bound+slack with the
...
fn arm3_backstop_stalled_handler_exits_within_bound() -> Result<(), Box<dyn Error>> {
    ...
    let mut daemon = start_daemon(
        ...
        &[
            ("QRMUX_CLAIM_TIMEOUT_MS".into(), "60000".into()),
            ("QRMUX_TEST_SLOW_DROP_MS".into(), "60000".into()),
            ("QRMUX_EXIT_WAIT_MS".into(), "500".into()),
        ],
        true,
    )?;
    let socket = daemon.socket.clone();
...
fn double_unlink_successor_socket_survives_old_daemon_exit() -> Result<(), Box<dyn Error>> {
    ...
    let mut old = start_daemon(
        ...
        &[
            ("QRMUX_CLAIM_TIMEOUT_MS".into(), "60000".into()),
            ("QRMUX_TEST_SLOW_DROP_MS".into(), "1500".into()),
        ],
        true,
    )?;
