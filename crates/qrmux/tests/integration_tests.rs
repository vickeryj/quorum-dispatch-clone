//! qrmux gate pass (a) integration suite — B1 stress cases as REAL tests.
//!
...
/// Evidence directory for a scenario: crates/qrmux/target/test-evidence/<runid>/<scenario>.
...
/// see doc/inbox/2026-06-10-qrmux-phone-scroll-regression.md). Asserts
...
fn g3_winch_storm() -> Result<(), Box<dyn Error>> {
    ...
    if cfg!(target_os = "macos")
        && std::env::var("QRMUX_CI_QUARANTINE")
            .map(|v| v.contains("g3-macos"))
            .unwrap_or(false)
    {
        eprintln!(
            "G3 QUARANTINED on macOS CI (QRMUX_CI_QUARANTINE=g3-macos):              see crates/qrmux/GATE-B2.md 'Quarantine' for the evidence chain.              NOT a pass on this platform-lane; macOS proof is the local lane."
        );
        return Ok(());
    }
...
///
/// `QRMUX_SOAK_LINES` overrides the line count for DEV iteration only — the
...
fn g5_soak() -> Result<(), Box<dyn Error>> {
    ...
    let lines: usize = std::env::var("QRMUX_SOAK_LINES")
        .ok()
...
fn leak_guard_eval(
    ...
    fs::write(
        ...
        format!(
            ...
            if injected { "QRMUX_TEST_LEAK=retain" } else { "NO (control-of-the-control)" },
            q2_mean,
            q4_mean,
            q4_mean - q2_mean,
...
/// (`QRMUX_TEST_LEAK=retain`: the daemon keeps a copy of every PTY chunk,
...
fn leak_guard_negative_control_fires() -> Result<(), Box<dyn Error>> {
    ...
    let mut env = jail_env(&jail);
    env.push(("QRMUX_TEST_LEAK".to_string(), "retain".to_string()));
...
fn leak_guard_no_retention_control_quiet() -> Result<(), Box<dyn Error>> {
    ...
    let env = jail_env(&jail); // NO QRMUX_TEST_LEAK
...
async fn version_negotiation_old_client_refused() -> Result<(), Box<dyn Error>> {
    use qrmux::protocol::codec::FrameReader;
    use qrmux::protocol::handshake::PREAMBLE_MAGIC;
...
async fn version_negotiation_modern_client_clean_error() -> Result<(), Box<dyn Error>> {
    use qrmux::protocol::{self, ServerMsg};
...
/// Spawn a REAL `qrmux attach` client process on its own PTY, kill -9 it,
...
fn detach_by_construction_kill9() -> Result<(), Box<dyn Error>> {
    ...
    let mut cmd = CommandBuilder::new(qrmux_binary());
...
fn d1_socket_dir_override_crosses_process_boundary() -> Result<(), Box<dyn Error>> {
    use libmod::client::{qrmux_binary, sweep_orphan_daemons, DaemonGuard};
    use std::process::{Command, Stdio};

    let jail = setup_jail("d1_socketdir_override")?;
    let env = jail_env(&jail);

    // An EXPLICIT socket dir that is NOT $XDG_RUNTIME_DIR/qrmux (the env tier).
    ...
    // daemon is PER-SESSION now (binds `<dir>/<name>.sock`, not `qrmux.sock`).
    ...
    cmd_env.push(("QRMUX_CLAIM_TIMEOUT_MS".into(), "60000".into()));
    let daemon = Command::new(qrmux_binary())
        .arg("server")
        .arg("--socket-dir")
        .arg(&explicit_dir)
        .arg("--session")
    ...
    let sessions = rt.block_on(qrmux::client::discovery::scan_sessions(dir))?;
    let info = sessions
        .iter()
        .find(|s| s.name == name)
    ...
    let history = rt.block_on(async {
        let deadline = Instant::now() + Duration::from_secs(10);
        loop {
            let h = qrmux::client::session_client::get_history_session(dir, &name)
                .await
    ...
    rt.block_on(async {
        qrmux::client::session_client::send_input_session(
            dir,
            None,
            &name,
    ...
    rt.block_on(qrmux::client::session_client::kill_session_session(
        dir, &name,
    ))
    .ok();
