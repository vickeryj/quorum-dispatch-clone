//! Exit-hint UX harness — drive the real `qrmux` client in a PTY and assert the
...
//! runs an `qrmux open` → detach flow that exercises the client's teardown path
...
/// Drive `qrmux open <session>` in a pty against an already-running per-session
...
fn capture_attach_then_detach() -> Vec<u8> {
    ...
    let mut cmd = CommandBuilder::new(libmod::client::qrmux_binary());
    cmd.env_clear();
    for (k, v) in &jail.env {
        cmd.env(k, v);
    }
    ...
    let mut child = pair.slave.spawn_command(cmd).expect("spawn qrmux open");
...
/// Drive `qrmux open <session>` in a pty, attach, then resize the pty master to
