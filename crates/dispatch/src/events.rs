...
/// lowercase hex of SHA-256, always"). Uses the existing `sha2` dep, same as
/// render.rs / codes.rs.
...
/// The 10 engine event payloads (§2.3), EXACT field names/types/optionality.
///
...
pub enum Payload {
    ...
    SendInitiated {
        ...
        /// ([`crate::redact::redact_for_preview`]). Additive (CR-1): appended LAST
        /// in the struct AND last in the pinned key order; omitted when None (an
        /// unresolvable text, or shrunk away by the §4.2 belt). NEVER raw content —
        /// API-key-shaped tokens + long runs are scrubbed.
...
fn insert_opt_str(obj: &mut Map<String, Value>, key: &str, v: &Option<String>) {
...
fn build_record_line(env: &Envelope, payload: &Payload, sha_cap: usize) -> String {
    let mut obj = Map::new();
