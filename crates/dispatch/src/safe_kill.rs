...
mod tests {
    ...
    fn noop_for_zero_one_negone_then_reaps_real_group() {
        ...
        assert!(pid_alive(child_pid), "sentinel must be alive before the test");
        ...
        assert!(pid_alive(child_pid), "sentinel must still be alive after the no-ops");
        ...
        assert!(pid_alive(self_pid), "test process must survive the real group kill");
    }
