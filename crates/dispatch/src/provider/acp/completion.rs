...
mod tests {
    ...
    fn jsonrpc_error_is_failed_not_completed() {
        ...
        let c = AcpTurnCompletion::new(&o);
        assert_eq!(c.poll_completion(), TurnCompletionProbe::Visible, "a failed turn IS done");
        let (reason, conf) = c.terminal_reason().unwrap();
        assert_eq!(reason, TerminalReason::Failed, "error terminal must be Failed, NOT Completed");
    ...
    fn transport_lost_is_degraded() {
        ...
        let c = AcpTurnCompletion::new(&o);
        assert!(matches!(c.poll_completion(), TurnCompletionProbe::Degraded(_)));
        assert_eq!(c.terminal_reason().unwrap().0, TerminalReason::TransportLost);
    }
