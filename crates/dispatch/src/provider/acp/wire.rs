...
fn decode_init(v: &Value) -> InitializeResult {
    InitializeResult {
        protocol_version: v.get("protocol_version").and_then(Value::as_i64).unwrap_or(1),
        agent_name: v
            .get("agent_name")
            .and_then(Value::as_str)
            .map(str::to_string),
...
impl AcpClient for AcpConnection {
    fn initialize(&self) -> Result<InitializeResult, AcpError> {
        self.request("initialize", json!({})).map(|v| decode_init(&v))
    }
    ...
    fn next_update(&self, timeout: Duration) -> Result<Option<AcpEvent>, AcpError> {
        ...
        let out = self.request("next_update", json!({ "timeout_ms": timeout.as_millis() as u64 }));
...
mod tests {
    ...
    fn wire_relays_event_payload_verbatim() {
        ...
        with_server(resident, |url| {
            ...
            assert_eq!(client.next_update(Duration::from_millis(300)).unwrap(), None);
        });
    }
