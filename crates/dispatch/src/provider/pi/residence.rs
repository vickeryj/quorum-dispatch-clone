...
mod tests {
    ...
    fn dropped_agent_end_is_reconciled_by_the_is_streaming_edge() {
        ...
        assert!(t.streaming, "event path alone: still (wrongly) believed busy");
        assert_eq!(t.completed, 0, "event path alone: the completion is uncounted");
        ...
        assert_eq!(t.completed, 1, "reconciled: counted exactly once, not missed");
    }
    ...
    fn rt_a_dropped_end_back_to_back_undercounts_never_overcounts() {
        ...
        assert_eq!(t.completed, 1, "ACCEPTED residual: 2 turns undercount to 1 (never 2+ overcount)");
    }
    ...
    fn rt_a_dropped_end_with_observed_idle_still_counts_both() {
        ...
        assert!(t.observe(false), "the backstop point-read counts turn 1's real close");
        assert_eq!(t.completed, 1);
        t.observe_event(&PiEvent::AgentStart); // turn 2 (invalidates the pending dedup)
        assert!(t.observe_event(&PiEvent::AgentEnd { will_retry: false }));
        assert_eq!(t.completed, 2, "genuine idle-gapped turns are NOT lost — only the no-idle corner is");
    }
    ...
    fn rt_b_dropped_start_then_delivered_end_now_counts() {
        ...
        assert_eq!(t.completed, 1, "FIXED: dropped-start + delivered end counts 1 (was 0)");
    ...
    fn rt_c_willretry_pointread_idle_blip_is_suppressed_no_overcount() {
        ...
        assert!(t.observe_event(&PiEvent::AgentEnd { will_retry: false }));
        assert_eq!(t.completed, 1, "one logical turn counted once (was 2 = overcount)");
    }
    ...
    fn residual_double_unobserved_subgap_turn_is_uncounted_but_status_truthful() {
        ...
        assert_eq!(t.completed, 0, "documented residual: double-unobserved sub-gap turn");
    ...
    fn rt_d_identity_guard_exact_token_not_substring() {
        ...
        assert!(!cmdline_is_our_pi_daemon(Some(live), Some("ws://127.0.0.1:900")));
        // Exact-port still matches (the common case is preserved).
        assert!(cmdline_is_our_pi_daemon(Some(live), Some("ws://127.0.0.1:9001")));
    ...
    fn rt2_g_reopen_does_not_defeat_terminal_dedup_now_counts_two() {
        ...
        // pending dedup.
        assert!(!t.observe(true));
        assert_eq!(t.pending_delayed_terminals, 1, "the re-open must NOT clear the pending dedup");
        ...
        assert_eq!(t.completed, 1, "no premature second count (was 2 = the G overcount)");
    ...
    fn rt2_g_dropped_end_then_pointread_reopen_then_delivered_turn2_counts_two() {
        ...
        assert!(!t.observe_event(&PiEvent::AgentStart));
        assert_eq!(t.pending_delayed_terminals, 0, "a start invalidates a stale pending dedup");
        assert!(t.observe_event(&PiEvent::AgentEnd { will_retry: false }));
        assert_eq!(t.completed, 2, "turn2's terminal counts — the dropped-end path is NOT regressed");
    }
    ...
    fn rt2_e1_documented_residual_leaked_dedup_undercounts() {
        ...
        assert!(!t.observe_event(&PiEvent::AgentEnd { will_retry: false }));
        assert_eq!(t.completed, 1, "DOCUMENTED RESIDUAL: compound adjacent drops undercount 2→1");
        assert!(!t.streaming, "status truthful (idle) — the GATE is not wedged");
    }
    ...
    fn rt2_e2_documented_residual_erased_boundary_undercounts() {
        ...
        assert!(t.observe_event(&PiEvent::AgentEnd { will_retry: false }));
        assert_eq!(t.completed, 1, "DOCUMENTED RESIDUAL: erased boundary undercounts 2→1");
    ...
    fn rt2_f_terminal_after_terminal_counts_favoring_reachable_dropped_start() {
        ...
        assert_eq!(reachable.completed, 2, "reachable dropped-start-after-a-turn must count 2");
    ...
    fn rt4_h_dropped_retry_boundary_no_longer_overcounts() {
        ...
        assert_eq!(t.completed, 1, "FIXED: one retrying turn counts 1 (was 2 = the H overcount)");
    }
    ...
    fn rt4_h_no_overcount_with_faithful_pointreads() {
        ...
        assert!(t.observe_event(&PiEvent::AgentEnd { will_retry: false }));
        assert_eq!(t.completed, 1, "faithful point-reads do not disambiguate → still 1, no overcount");
    }
    ...
    fn rt4_h_control_dropped_resume_only_counts_one() {
        ...
        assert!(t.observe_event(&PiEvent::AgentEnd { will_retry: false }));
        assert_eq!(t.completed, 1, "boundary armed retry_pending; the terminal counts the one turn once");
    }
    ...
    fn never_overcount_across_the_full_matrix() {
        ...
        let cases: &[(&str, u64, u64, Drv)] = &[
            ("clean 1", 1, 1, |t| { t.observe_event(&PiEvent::AgentStart); t.observe_event(&PiEvent::AgentEnd { will_retry: false }); }),
            ("clean 3", 3, 3, |t| { for _ in 0..3 { t.observe_event(&PiEvent::AgentStart); t.observe_event(&PiEvent::AgentEnd { will_retry: false }); } }),
            ("H dropped-boundary", 1, 1, |t| { t.observe_event(&PiEvent::AgentStart); t.observe_event(&PiEvent::AgentStart); t.observe_event(&PiEvent::AgentEnd { will_retry: false }); }),
            ("A dropped-end back-to-back", 2, 1, |t| { t.observe_event(&PiEvent::AgentStart); t.observe_event(&PiEvent::AgentStart); t.observe_event(&PiEvent::AgentEnd { will_retry: false }); }),
            ("A dropped-end WITH idle", 2, 2, |t| { t.observe_event(&PiEvent::AgentStart); t.observe(false); t.observe_event(&PiEvent::AgentStart); t.observe_event(&PiEvent::AgentEnd { will_retry: false }); }),
            ("B dropped-start", 1, 1, |t| { t.observe_event(&PiEvent::AgentEnd { will_retry: false }); }),
            ("C retry clean", 1, 1, |t| { t.observe_event(&PiEvent::AgentStart); t.observe_event(&PiEvent::AgentEnd { will_retry: true }); t.observe(false); t.observe_event(&PiEvent::AgentStart); t.observe_event(&PiEvent::AgentEnd { will_retry: false }); }),
            ("E1 compound", 2, 1, |t| { t.observe_event(&PiEvent::AgentStart); t.observe(false); t.observe_event(&PiEvent::AgentEnd { will_retry: false }); }),
            ("E2 compound", 2, 1, |t| { t.observe_event(&PiEvent::AgentStart); t.observe_event(&PiEvent::AgentEnd { will_retry: false }); }),
            ("G delayed-terminal+reopen", 2, 2, |t| { t.observe_event(&PiEvent::AgentStart); t.observe(false); t.observe(true); t.observe_event(&PiEvent::AgentEnd { will_retry: false }); t.observe_event(&PiEvent::AgentStart); t.observe_event(&PiEvent::AgentEnd { will_retry: false }); }),
        ];
        for (name, real, want, drv) in cases {
            let mut t = TurnTracker::default();
            drv(&mut t);
            assert!(t.completed <= *real, "{name}: OVERCOUNT {} > {real} real turns", t.completed);
            assert_eq!(t.completed, *want, "{name}: expected {want}, got {}", t.completed);
        }
    }
    ...
    fn twelve_turn_mixed_accumulation_never_overcounts() {
        let mut t = TurnTracker::default();
        let start = |t: &mut TurnTracker| { t.observe_event(&PiEvent::AgentStart); };
        let end = |t: &mut TurnTracker| { t.observe_event(&PiEvent::AgentEnd { will_retry: false }); };
        for i in 0..12u64 {
            match i % 4 {
                0 => { start(&mut t); end(&mut t); }                                   // clean
                1 => { end(&mut t); }                                                  // B: dropped start
                2 => { start(&mut t); t.observe(false); }                              // pr-close (dropped end, idle observed)
                _ => { start(&mut t); t.observe_event(&PiEvent::AgentEnd { will_retry: true }); start(&mut t); end(&mut t); } // retry-turn
            }
            assert!(t.completed <= i + 1, "step {i}: overcount {} > {}", t.completed, i + 1);
        }
        assert_eq!(t.completed, 12, "12 counted turns accumulate exactly, no overcount");
    }
}
...
mod sustained_load {
    ...
    use crate::provider::pi::rpc::PiRpc; // brings `next_event` into scope on &PiStdio
    use crate::provider::pi::stdio::{PiStdio, CHANNEL_CAPACITY};
    use std::cell::RefCell;
    use std::path::Path;
    use std::time::{Duration, Instant};
    ...
    ///     provider::pi::residence::sustained_load -- --nocapture --test-threads=1
    #[test]
    ...
    fn forced_channel_drop_of_agent_end_is_reconciled_by_a_point_read() {
        ...
        assert!(counted, "the busy→idle edge fires on the point-read");
        let t = tracker.borrow();
        assert!(!t.streaming, "reconciled: NOT stuck-busy after a dropped agent_end");
        assert_eq!(t.completed, 1, "reconciled: counted exactly once, not missed");
