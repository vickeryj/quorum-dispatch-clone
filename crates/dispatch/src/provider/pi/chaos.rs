...
use super::residence::cmdline_is_our_pi_daemon;
...
impl ChaosReport {
    ...
    pub fn clean(&self) -> bool {
        !self.verdicts.is_empty()
            && self.verdicts.iter().all(|v| v.pass && !v.is_break)
    }
    ...
    pub fn characterizations(&self) -> Vec<&ChaosVerdict> {
        self.verdicts.iter().filter(|v| v.is_characterization).collect()
    }
...
impl GroupReaper {
    fn new() -> Self {
        Self { pgids: Vec::new(), pids: Vec::new(), own_pgid: own_pgid() }
    }
...
fn check_kill_safe(
    ...
    if pgid == own_pgid {
        return Err(format!("layer-a(iii): refuse group-kill of pgid={pgid} == executor own pgid"));
    }
...
fn class1_reconnect(runner: &QdRunner, cwd: &str, dir: &Path, guard: &mut KillGuard) -> Vec<ChaosVerdict> {
    ...
    let started_ok = started.as_ref().map(|o| o.status.success()).unwrap_or(false);
    let row = runner.read_row(name);
    let (pid, endpoint, started_at) = match &row {
        Some((_, v)) => (
            v.get("pid").and_then(Value::as_i64),
            v.get("endpoint").and_then(Value::as_str).map(str::to_string),
    ...
    let info_pid = info_json.as_ref().and_then(|v| v.get("pid").and_then(Value::as_i64));
    let info_live = info_json.as_ref().and_then(|v| v.get("live").and_then(Value::as_bool));
    ...
    let ls_has_name = ls
        ...
        .unwrap_or(false);
    log.push_str(&format!("qd ls --live --json contains {name}: {ls_has_name}\n"));

    // Endpoint RE-RESOLVED from the registry row (the §9.4-faithful source).
    let row_endpoint = runner
        .read_row(name)
        .and_then(|(_, v)| v.get("endpoint").and_then(Value::as_str).map(str::to_string));
    log.push_str(&format!("endpoint re-resolved from row: {row_endpoint:?}\n"));
    ...
    let daemon_alive = super::daemon::pi_daemon_is_alive(
        pid,
        row_endpoint.as_deref(),
        &alive_i64,
        &probe,
    );
    let identity = cmdline_is_our_pi_daemon(real_cmdline_probe(pid).as_deref(), row_endpoint.as_deref());
    let ws_ok = match &row_endpoint {
        Some(ep) => super::residence::connect_ready(ep, Duration::from_secs(5)).is_ok(),
        None => false,
    };
    log.push_str(&format!(
    ...
    log.push_str(&format!("exactly-one-live(name): {one_live} (resident pgid members={group_members})\n"));
...
fn class2_crash_rebind(runner: &QdRunner, cwd: &str, dir: &Path, round: u32, guard: &mut KillGuard) -> Vec<ChaosVerdict> {
    ...
    let (Some(pid_a), endpoint_a, started_at_a) = (
        row_a.as_ref().and_then(|(_, v)| v.get("pid").and_then(Value::as_i64)),
        row_a.as_ref().and_then(|(_, v)| v.get("endpoint").and_then(Value::as_str).map(str::to_string)),
        row_a.as_ref().and_then(|(_, v)| v.get("startedAt").and_then(Value::as_i64)),
    ) else {
        write_at_source(dir, class, &log);
    ...
    let ws_after = endpoint_a
        .as_deref()
        .map(|ep| super::residence::connect_ready(ep, Duration::from_secs(1)).is_ok())
        .unwrap_or(false);
    let identity_after =
    ...
    registry::ensure_tombstone(&sessions_dir, pid_a, row_a.as_ref().map(|(_, v)| {
    ...
    let pid_b = row_b.as_ref().and_then(|(_, v)| v.get("pid").and_then(Value::as_i64));
    let endpoint_b = row_b
        .as_ref()
        .and_then(|(_, v)| v.get("endpoint").and_then(Value::as_str).map(str::to_string));
    let started_at_b = row_b.as_ref().and_then(|(_, v)| v.get("startedAt").and_then(Value::as_i64));
    ...
    let mut out = vec![v
        .cmd(format!("qd {}", start.join(" ")))
        .cmd(format!("safe_group_kill({pid_a}, SIGKILL)  (abrupt group crash, bypass qd stop)"))
    ...
    if round >= 2 {
        ...
        let pid_c = row_c.as_ref().and_then(|(_, v)| v.get("pid").and_then(Value::as_i64));
        if let Some(pid_c) = pid_c {
            ...
            let pid_re = row_re.as_ref().and_then(|(_, v)| v.get("pid").and_then(Value::as_i64));
            let reclaimed = reok && pid_re.is_some() && pid_re != Some(pid_c)
            ...
            out.push(vr.cmd(format!("qd start {rname} (x2, crash between)")).obs(rlog));
        } else {
            let _ = runner.run(&["stop", rname]);
            out.push(ChaosVerdict::brk(class, dir, "class2' rebind: no resident row/pid after start (instrument)").obs(rlog));
        }
    }
...
fn class3_wedge(runner: &QdRunner, cwd: &str, dir: &Path, live: bool, round: u32, guard: &mut KillGuard) -> Vec<ChaosVerdict> {
    ...
    let v3a = if pass_3a {
        ...
        ChaosVerdict::brk(class, dir, "daemon-gone wait HUNG (exceeded bound) — degrade-not-crash violated")
    } else {
        ChaosVerdict::brk(class, dir, "daemon-gone wait PANICKED — degrade-not-crash violated")
    };
    out.push(v3a.cmd(format!("qd {}", wait_args.join(" "))).obs(log.clone()));
    ...
    if round >= 2 {
        ...
        let v3ab = if bpanicked {
            ChaosVerdict::brk(class, dir, "busy dead-pid wait PANICKED — degrade-not-crash violated")
        } else if !bounded {
            ChaosVerdict::brk(class, dir, "busy dead-pid wait HUNG past bound — degrade-not-crash violated")
        } else if !polled {
    ...
    out.push(ChaosVerdict::characterization(class, dir, "transcript-root divergence pinned for the acceptance oracle").obs(cite.to_string()));
    ...
    if live {
        ...
        if let Some(pid_b) = pid_b {
            ...
            if let Some(child) = &child {
                ...
                wlog.push_str(&format!("SIGSTOP child pid={} comm={}\n", child.pid, child.comm));
            }
            ...
            let v3b = if pass_3b {
                ...
                ChaosVerdict::brk(class, dir, "live wedge: qd wait HUNG past bound — degrade-not-crash violated")
            } else {
                ChaosVerdict::brk(class, dir, "live wedge: qd wait PANICKED — degrade-not-crash violated")
            };
            out.push(
            ...
            let _ = runner.run(&["stop", name_b]);
            out.push(ChaosVerdict::brk(class, dir, "live wedge: no resident row/pid after start").obs(wlog));
        }
    }
...
fn class4_pgid_teardown(runner: &QdRunner, cwd: &str, dir: &Path, live: bool, round: u32, guard: &mut KillGuard) -> Vec<ChaosVerdict> {
    ...
    if live {
        ...
        let _ = runner.run(&start);
        let row = runner.read_row(name);
        let pid = row.as_ref().and_then(|(_, v)| v.get("pid").and_then(Value::as_i64));
        if let Some(pid) = pid {
            ...
            if round >= 2 {
                match Command::new("sleep").arg("600").process_group(pid as i32).spawn() {
                    Ok(child) => {
            ...
            let survivors: Vec<&ProcRow> = after.iter().filter(|r| is_pid_alive(r.pid as i32)).collect();
            ...
            out.push(
                v4a.cmd(format!("qd {}", start.join(" ")))
                    .cmd(format!("qd stop {name}  (→ teardown_pi_daemon → -pgid SIGTERM→grace→SIGKILL)"))
                    .obs(log),
            );
        } else {
            let _ = runner.run(&["stop", name]);
            out.push(ChaosVerdict::brk(class, dir, "class4a: no resident row/pid after start").obs(log));
        }
    }

    // --- 4b: setsid escapee reach-boundary (always-on characterization) ------
    write_ran(dir, "class4b setsid escapee reach-boundary characterization");
...
fn class4b_escapee(dir: &Path, round: u32, guard: &mut KillGuard) -> ChaosVerdict {
    ...
    let mut root = match root {
        ...
        Err(e) => {
            ...
            return ChaosVerdict::brk(class, dir, "class4b: could not spawn the escapee model (instrument)").obs(log);
        }
    };
    ...
    let Some(escapee_pid) = escapee_pid else {
        ...
        let _ = root.wait();
        write_at_source(dir, "class4b", &log);
        return ChaosVerdict::brk(class, dir, "class4b: escapee never detached (instrument)").obs(log);
    };
    ...
    log.push_str(&format!("ESCAPE PROVEN (root_pgid != escapee_pgid): {escape_proven}\n"));
    ...
    let ingroup_child: Option<i64> = if round >= 2 {
        let m = pgid_members(root_pid)
            ...
            .map(|r| r.pid);
        log.push_str(&format!("IN-GROUP child (round {round} combined topology) = {m:?}\n"));
        m
    } else {
        None
    };
    ...
    guarded_kill(guard, root_pid, "class4b-root", || RealDaemonSpawner.kill(root_pid));
    let _ = root.wait();
    ...
    v.cmd(format!("sh -c <setsid-escape script>; process_group(0); root_pid={root_pid}"))
        .cmd(format!("RealDaemonSpawner.kill({root_pid})  (-pgid SIGTERM→grace→SIGKILL)"))
        .obs(log)
}
...
pub fn run_chaos_round(
    ...
    if live {
        verdicts.extend(class1_reconnect(runner, cwd, &class_dir(evidence_root, "class1-reconnect"), &mut guard));
        verdicts.extend(class2_crash_rebind(runner, cwd, &class_dir(evidence_root, "class2-crash-rebind"), round, &mut guard));
    }
    verdicts.extend(class3_wedge(runner, cwd, &class_dir(evidence_root, "class3-wedge"), live, round, &mut guard));
    verdicts.extend(class4_pgid_teardown(runner, cwd, &class_dir(evidence_root, "class4-pgid-teardown"), live, round, &mut guard));
    ...
    ChaosReport { round, live, verdicts }
}
...
mod tests {
    ...
    fn clean_round_requires_nonempty_no_break() {
        ...
        let mut r = ChaosReport { round: 1, live: true, verdicts: vec![ChaosVerdict::pass("c", dir, "ok")] };
        assert!(r.clean());
        // A characterization does NOT dirty the round.
        r.verdicts.push(ChaosVerdict::characterization("c", dir, "pinned"));
        assert!(r.clean());
        // A break DOES.
        ...
        // Empty is not clean.
        let empty = ChaosReport { round: 1, live: true, verdicts: vec![] };
        assert!(!empty.clean());
    }
    ...
    fn layer_a_i_refuses_pgid_leq_1() {
        ...
        let spawned = set(&[]);
        let fleet = set(&[]);
        assert!(check_kill_safe(1, 1234, &spawned, &fleet).unwrap_err().contains("layer-a(i)"));
        assert!(check_kill_safe(0, 1234, &spawned, &fleet).unwrap_err().contains("layer-a(i)"));
        assert!(check_kill_safe(-5, 1234, &spawned, &fleet).unwrap_err().contains("layer-a(i)"));
    }
