...
use super::session::encode_cwd_dir;
...
pub fn encode_cwd_dir_shape_ok() -> (bool, String) {
    ...
    if bad.is_empty() {
        (
            true,
            format!("encode_cwd_dir matches the PA5 `--<enc>--` shape + regex on {} cases", cases.len()),
        )
    } else {
...
pub fn run_tier_a(runner: &QdRunner, d_ts_path: Option<&Path>) -> ConformanceReport {
    ...
    let started_ok = start_out.as_ref().map(|o| o.status.success()).unwrap_or(false);
    ...
    items.push(ItemResult {
        ...
        detail: if started_ok && row1.is_some() {
            "start --provider pi succeeded; row written (resident reached get_state readiness)".into()
        } else {
    ...
    let info_ok = info_out.as_ref().map(|o| o.status.success()).unwrap_or(false);
    let addr_ok = row1.as_ref().is_some_and(|(_, v)| {
        v.get("name").and_then(Value::as_str) == Some(s1)
            && v.get("sessionId").and_then(Value::as_str).is_some_and(|s| !s.is_empty())
            && v.get("endpoint").and_then(Value::as_str).is_some_and(|s| !s.is_empty())
            && v.get("pid").and_then(Value::as_i64).is_some()
    }) && info_ok;
    items.push(ItemResult {
        item: 2,
        pass: addr_ok,
        detail: "row carries name+sessionId(birth-id)+endpoint+pid AND `qd info <name>` resolves".into(),
        commands: vec![format!("qd {}", start1.join(" ")), format!("qd {}", info1.join(" "))],
        observed: format!("row={row1_json}\n{}", info_out.as_ref().map(|o| out_evidence(&info1, o)).unwrap_or_default()),
    });
    ...
    let (transport_ok, transport_obs) = match &row1 {
        Some((_, v)) => {
            ...
            (ours, format!("pid={pid} cmdline={:?} endpoint={:?}", cmdline, endpoint))
        }
    ...
    let concurrency_ok = start2_out.as_ref().map(|o| o.status.success()).unwrap_or(false)
        && match (&row1, &row2) {
            (Some((_, a)), Some((_, b))) => {
    ...
    items.push(ItemResult {
        ...
        detail: "`qd ls` succeeds and the resident pid is alive (liveness reflects the real process)".into(),
        commands: vec!["qd ls".to_string()],
        observed: ls_out.as_ref().map(|o| out_evidence(&ls, o)).unwrap_or_default(),
    });
    ...
    let (m_pass, m_detail, m_obs) = match d_ts_path {
        Some(dts) => match pin::run_pin_smoke(&runner.pi_bin, dts) {
            Ok(r) => (r.ok(), format!("pin smoke ok={}; {:?}", r.ok(), r.failures()), format!("{r:?}")),
            Err(e) => (false, format!("pin smoke exec error: {e}"), e),
        },
        None => (false, "no rpc-types.d.ts path provided (cannot run the shape/hash pin)".into(), String::new()),
    };
    items.push(ItemResult {
        item: 12,
        pass: m_pass,
    ...
    let pid1 = row1.as_ref().and_then(|(_, v)| v.get("pid").and_then(Value::as_i64));
...
mod tests {
    ...
    fn report_green_requires_nonempty_all_pass_and_serializes() {
        ...
        let mixed = ConformanceReport {
            items: vec![
                ItemResult { item: 1, pass: true, detail: "ok".into(), commands: vec![], observed: "x".into() },
                ItemResult { item: 2, pass: false, detail: "boom".into(), commands: vec![], observed: "y".into() },
            ],
        };
