...
pub fn version_matches(version_output: &str) -> bool {
    version_output.trim().split_whitespace().any(|t| t == PINNED_VERSION)
...
pub fn run_pin_smoke(pi_bin: &Path, d_ts_path: &Path) -> Result<PinReport, String> {
    ...
    let bytes = std::fs::read(d_ts_path)
        .map_err(|e| format!("read {}: {e}", d_ts_path.display()))?;
...
"#;
    ...
    fn protocol_version_detection() {
        ...
        assert!(has_protocol_version("interface X { protocolVersion: number }"));
    }
