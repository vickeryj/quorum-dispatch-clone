...
///
/// Event → [`super::republish::Republish`] mapping (retained for A2's status
...
pub enum PiEvent {
    ...
    MessageUpdate { assistant_event: Option<AssistantEvent> },
...
pub fn classify(v: &Value) -> Option<Frame> {
    ...
    let event = match kind {
        ...
        "message_update" => PiEvent::MessageUpdate {
            assistant_event: obj.get("assistantMessageEvent").map(classify_assistant_event),
        },
...
pub trait PiRpc {
    ...
    fn prompt(&self, message: &str, behavior: Option<StreamingBehavior>)
...
mod tests {
    ...
    const EV_ERROR: &str =
        r#"{"type":"response","command":"parse","success":false,"error":"Unexpected token u in JSON"}"#;
    ...
    fn classifies_the_three_sink_events() {
        ...
        assert!(matches!(classify(&start), Some(Frame::Event(PiEvent::AgentStart))));
