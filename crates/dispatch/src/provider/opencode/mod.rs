...
mod tests {
    ...
    fn mint_store(dir: &TempDir, sessions: &[SessionSeed], messages: &[(&str, &str)]) -> PathBuf {
        ...
        for s in sessions {
            conn.execute(
                ...
                rusqlite::params![
                    s.id, s.slug, s.directory, s.title, s.version,
                    s.time_created_ms, s.time_updated_ms,
                    s.tokens_input, s.tokens_cache_read, s.tokens_cache_write
                ],
            )
            .unwrap();
        }
    ...
    fn reads_real_schema_rows_with_turns_and_tokens() {
        ...
        let messages: Vec<(&str, &str)> =
            msg_ids.iter().map(|m| (m.as_str(), "ses_big")).collect();
        mint_store(
            &tmp,
            &[seed("ses_big", "/home/u", "Testing functionality check", 147_745)],
            &messages,
        );
    ...
    fn turns_count_message_not_session_message() {
        ...
        let rows = sessions(tmp.path());
        assert_eq!(rows.len(), 1);
        assert_eq!(rows[0].turns, 2, "turns come from message, not session_message");
    }
    ...
    fn garbage_db_is_empty() {
        ...
        .unwrap();
        assert!(sessions(tmp.path()).is_empty(), "a non-db blob degrades to empty");
    }
    ...
    fn malformed_row_skipped_siblings_survive() {
        ...
        drop(conn);
        let rows = sessions(tmp.path());
        assert_eq!(rows.len(), 1, "the malformed row is dropped, the good one survives");
