...
mod tests {
    ...
    fn fold_skips_observed_lines() {
        ...
        let observed =
            build_observed_line("t", "qrmoh", "claude", "sid-obs", Some("/w"));
        let create = build_create_line(
            "t",
            &CreateEvent {
                name: "real".into(),
    ...
    fn record_observed_first_sighting_appends_once_then_noops() {
        ...
        assert!(dir.path().join("observed-claims").join("qrmoh~claude~sid-1").exists());
    ...
    fn record_observed_failed_append_does_not_poison_claim() {
        ...
        assert!(!dir.path().join("observed-claims").join("qrmoh~claude~sid-2").exists());
    ...
    fn record_observed_writes_no_registry_row() {
        ...
        let clock = FixedClock(1);
        let h = RecordHooks::default();
        record_observed_in(dir.path(), &clock, "qrmoh", "claude", "sid-3", Some("/w"), &h)
            .unwrap();
        ...
        assert!(dir.path().join("observed-claims").join("qrmoh~claude~sid-3").exists());
    }
    ...
    fn deobs_failed_append_leaves_identity_recordable() {
        ...
        let r = record_observed_in(dir.path(), &clock, "h", "claude", "sid-clause2", None, &fail);
        assert!(r.is_err(), "forced append failure surfaces as a non-fatal Err");
        assert_eq!(
    ...
    fn deobs_first_sighting_volume_exactly_one_line_per_key() {
        ...
        assert_eq!(
            record_observed_in(dir.path(), &clock, "qrmoh", "claude", "sid-vol", Some("/w"), &h),
            Ok(true)
        );
        for _ in 0..500 {
            assert_eq!(
    ...
    fn deobs_creates_no_registry_row_sibling_registry_untouched() {
        ...
        record_observed_in(&state_dir, &clock, "qrmoh", "claude", "sid-reg", Some("/w"), &h)
            .unwrap();

        // The sibling registry dir is untouched: still empty.
        let entries: Vec<_> = std::fs::read_dir(&sessions_dir).unwrap().flatten().collect();
        assert!(
            entries.is_empty(),
