//! C1 M4 (D2): the engine's sbmux socket-dir resolver — the embedded-backend
//! analogue of [`crate::zmx_dir::resolve_zmx_dir`].
//!
//! Two tiers, NO literal `/tmp` (ADD-14: the engine never WRITES under literal
//! `/tmp`):
//!   1. `$XDG_RUNTIME_DIR/sbmux` (per-user, systemd-managed), else
...
//!
//! This mirrors sbmux's own standalone fallback (`server/socket.rs::resolve_socket_dir`)
...
/// socket-directory path. Mirrors sbmux's `SOCKET_LEAF_BUDGET` (the daemon places
...
/// budget is enforced separately by sbmux's `session_socket_path_for`).
...
/// sbmux::server::socket::SUN_PATH_MAX.
const SUN_PATH_MAX: usize = 104;

/// Resolve the engine's sbmux socket dir from the injected `home` + `Env` seam.
///
/// Tier 1 `$XDG_RUNTIME_DIR/sbmux` (trimmed; empty/whitespace-only treated as
...
pub fn resolve_sbmux_dir(home: &Path, env: &dyn Env) -> Result<PathBuf, String> {
    let dir = match nonblank(env.var("XDG_RUNTIME_DIR")) {
        // Tier 1: $XDG_RUNTIME_DIR/sbmux.
        Some(xdg) => PathBuf::from(xdg.trim()).join("sbmux"),
    ...
    // budget (`len(dir) + 1 + len(name) + 5 ≤ 104`) lives in sbmux's
...
mod tests {
    ...
    fn tier1_xdg_runtime_dir_wins() {
        ...
        let dir = resolve_sbmux_dir(Path::new("/jail/home"), &e).unwrap();
        assert_eq!(dir, PathBuf::from("/run/user/501/sbmux"));
    }

    #[test]
    fn tier1_xdg_trimmed() {
        // Whitespace-padded XDG value → trimmed, then joined with `sbmux`.
        let e = env(501, &[("XDG_RUNTIME_DIR", "  /run/user/501  ")]);
        let dir = resolve_sbmux_dir(Path::new("/jail/home"), &e).unwrap();
        assert_eq!(dir, PathBuf::from("/run/user/501/sbmux"));
    }
    ...
    fn tier2_sb_home_only_jail() {
        ...
        let dir = resolve_sbmux_dir(Path::new("/real/home"), &e).unwrap();
    ...
    fn guard_trips_on_overlong_tier2() {
        ...
        let e = env(501, &[]);
        let err = resolve_sbmux_dir(Path::new(&long_home), &e).unwrap_err();
        assert!(
    ...
    fn guard_trips_on_overlong_xdg() {
        ...
        let err = resolve_sbmux_dir(Path::new("/jail/home"), &e).unwrap_err();
    ...
    fn short_xdg_does_not_trip_guard() {
        let e = env(501, &[("XDG_RUNTIME_DIR", "/run/user/501")]);
        assert!(resolve_sbmux_dir(Path::new("/jail/home"), &e).is_ok());
    }
}
