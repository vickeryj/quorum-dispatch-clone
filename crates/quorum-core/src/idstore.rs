...
pub fn mint_or_get_with(
    ...
    let line = serde_json::json!({
        "v": 1,
        "ts": crate::timefmt::epoch_ms_to_iso(clock.now_ms()),
        "event": "mint",
        "id": id,
...
mod tests {
    ...
    fn acp_row_mints_and_reuses_stable_id_across_resume() {
        ...
        assert_eq!(
            first, again,
            "the same acp sessionId reuses its id (never reassigned)"
        );
        ...
        assert_eq!(
            first, after_resume,
            "the id is STABLE across stop→resume (same sessionId)"
        );
        assert_eq!(
    ...
    fn pi_create_flow_is_a_single_mint_and_the_ls_backfill_is_a_noop() {
        ...
        assert_eq!(
            mint_or_get(&path, birth, Some("pi-wk"), &clock).unwrap(),
            id
        );
        assert_eq!(
