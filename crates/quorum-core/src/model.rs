...
/// history) and `killed` (tombstoned) are dead — see [`is_live_status`] below
...
///
/// LIVES HERE, beside [`SessionStatus`], NOT in `dispatch::resolve` where it was
/// written (qd/qw split): this is a pure match on the enum — it says what the enum
/// MEANS, not how a target is resolved — and both qd and qw need the answer, so it
/// belongs in the `quorum-core` LEAF that neither owns. `resolve` re-exports it, so
/// `resolve::is_live_status` still names the same function.
...
///
/// Moved here with [`is_live_status`], for the same reason and by the same
/// re-export: the pid refinement is the other half of one answer, and splitting
/// the pair across crates would put the caller's `&&` in two homes. The
/// `pid_alive` seam it takes ([`crate::effects::is_pid_alive`]) was already here.
...
pub struct Session {
    ...
    /// This RAW field is still not serialized on `ls --json` — but since D1
    /// (`16-default-lane-switch.md` X3) what it resolves to is: render.rs emits a
    /// `lane` key carrying `lane_for(provider, hosting)`'s stable
    /// `<provider-id>/<hosting-token>` id. The derived answer is on the wire, the
    /// stored token is not, which is the same rule this doc states for consumers:
    /// ask the helper, never the string.
