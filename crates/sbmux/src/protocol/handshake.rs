...
/// Magic bytes identifying an sbmux client connection. Frozen forever.
...
pub enum PreambleCheck {
    ...
    Eof,
    /// First 4 bytes were not the sbmux magic — not an sbmux client.
    BadMagic([u8; 4]),
...
mod tests {
    ...
    /// the spec calls out (stale sbmux daemon ↔ new client). The adapter
    /// surfaces this PER-SESSION as "stale sbmux daemon for session '<name>';
