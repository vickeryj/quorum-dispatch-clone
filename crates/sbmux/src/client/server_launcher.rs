...
///
/// The standalone sbmux CLI re-execs `current_exe()` with `["server"]` (its own
/// binary HAS a `server` subcommand). But an EMBEDDING binary (the `sb` engine)
/// is the daemon too — it links sbmux — yet its daemon entry is a DIFFERENT
/// subcommand (`sb sbmux-server`), and `current_exe()` for it is the `sb` binary,
...
pub struct ServerLaunchSpec {
    ...
    pub program: PathBuf,
    /// Args BEFORE `--socket-dir` (e.g. `["sbmux-server"]` for the sb embedder,
    /// or `["server"]` to mirror the standalone default explicitly).
    pub args_prefix: Vec<String>,
}

impl ServerLaunchSpec {
    /// The standalone-CLI default: `current_exe() server` (the sbmux binary's own
...
// legacy shared-daemon `ensure_server_running[_with]` (which bound `sbmux.sock`)
...
/// `SBMUX_LAUNCH_BUDGET_MS` because N-way concurrent cold-start on a loaded box
...
fn launch_budget_from_env() -> std::time::Duration {
    std::env::var("SBMUX_LAUNCH_BUDGET_MS")
        .ok()
...
/// `sb sbmux-server`); `None` is the standalone default.
