...
//! (EXCLUDING the legacy `sbmux.sock`), and probes each socket with a connect,
...
const LEGACY_LEAF: &str = "sbmux.sock";
...
pub async fn scan_sessions(socket_dir: Option<&Path>) -> anyhow::Result<Vec<SessionInfo>> {
    ...
    // `sbmux.sock` (§4.3 step 1). Sequential probe order (named choice).
...
async fn probe_socket_inner(path: &Path, name: &str) -> ProbeOutcome {
    ...
    match read_frame(&mut frames, &mut stream).await {
        Some(ServerMsg::Hello { session, .. }) => {
            // G-ISOL negative-control seam (spec §7): under `SBMUX_TEST_SHARED=1`
            ...
            if session != name && !crate::server::socket::shared_fate_test_mode() {
                ...
                tracing::warn!(
                    leaf = %name,
                    actual = %session,
                    "sbmux discovery: socket leaf identity mismatch — skipping"
                );
                return ProbeOutcome::Skip;
            }
        }
...
/// Outcome of probing the legacy shared `sbmux.sock` (spec §5.3, orc rider W-4).
/// The engine maps `Live` to the one-line stderr visibility warning and `Stale`
/// to a per-target socket unlink; `Absent` is the common case (no legacy daemon).
#[derive(Debug, PartialEq)]
pub enum LegacyProbe {
    /// A LIVE daemon answered on `sbmux.sock` (any version — a pre-split v2
    /// daemon refuses our v3 preamble with a framed error, which is still a
    /// LIVE answer). Its sessions are invisible to this binary. NEVER auto-kill.
    Live,
    /// `ConnectionRefused`: a stale socket with no daemon behind it → eligible
    /// for per-target unlink (socket-only, same rule as §4.3).
    Stale,
    /// No legacy socket present (ENOENT) — the common case.
    Absent,
}

/// Probe the legacy shared `<dir>/sbmux.sock` (spec §5.3). Connect + write the
/// v3 preamble + Hello; ANY answer (including a framed version-mismatch refusal
/// from a pre-split v2 daemon) classifies the daemon LIVE. `ConnectionRefused`
/// is a stale socket (→ `Stale`); ENOENT/missing is `Absent`. This NEVER unlinks
/// and NEVER kills — it only classifies; the engine decides the §5.3 response.
///
/// Bounded by [`PROBE_TIMEOUT`]: a daemon that accepts the connection but never
/// answers (e.g. wedged) is still classified `Live` (present, do not touch) —
/// the conservative reading (never orphan a daemon that answered the connect).
pub async fn probe_legacy_socket(socket_dir: Option<&Path>) -> anyhow::Result<LegacyProbe> {
    let dir = socket_dir_for(socket_dir)?;
    let path = dir.join(LEGACY_LEAF);
    Ok(probe_legacy_inner(&path).await)
}

async fn probe_legacy_inner(path: &Path) -> LegacyProbe {
    let mut stream = match UnixStream::connect(path).await {
        Ok(s) => s,
        Err(e) if e.kind() == std::io::ErrorKind::ConnectionRefused => return LegacyProbe::Stale,
        // ENOENT (no legacy socket) or any other connect error → treat as absent
        // (the §5.3 window is dev-only; a connect error other than ECONNREFUSED
        // means there is nothing we can safely warn about or unlink).
        Err(_) => return LegacyProbe::Absent,
    };
    // The connect SUCCEEDED — a daemon (any version) is bound. Best-effort
    // handshake: a pre-split v2 daemon will read our v3 preamble and answer with
    // a framed version-mismatch refusal; a (hypothetical) v3 daemon on the legacy
    // leaf would answer Hello. Either way it is LIVE. We do not require a specific
    // reply — the successful connect already proves a live daemon; the handshake
    // is attempted only so the connection closes cleanly. A bind without an
    // acceptor would have refused the connect (→ Stale above).
    if protocol::write_preamble(&mut stream).await.is_ok() {
        if let Ok(hello) = protocol::encode(&ClientMsg::Hello { caps: vec![] }) {
            let _ = stream.write_all(&hello).await;
        }
        // Best-effort: read one frame (the refusal/Hello) under the bound, then
        // drop. We ignore the content — a live answer is all §5.3 needs.
        let mut frames = FrameReader::new();
        let _ = tokio::time::timeout(PROBE_TIMEOUT, read_frame(&mut frames, &mut stream)).await;
    }
    LegacyProbe::Live
}
