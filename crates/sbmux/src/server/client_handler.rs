...
pub struct DaemonCtx {
    ...
    /// the bare-sbmux binary / the test fixture (no headless support → the verb is
...
impl DaemonCtx {
    ...
    fn check_identity(&self, name: &str) -> Result<(), String> {
        ...
        // `SBMUX_TEST_SHARED=1` the capacity-1 gate accepts ANY session name so the
    ...
    match tokio::time::timeout_at(deadline, handshake::read_preamble(&mut stream)).await {
        ...
        Ok(Ok(PreambleCheck::BadMagic(m))) => {
            let resp = protocol::encode(&ServerMsg::Error(format!(
                "not an sbmux client (bad magic {:02x?})",
                m
            )))?;
        ...
        }
        {
            match msg {
                ...
                ClientMsg::LaunchHeadless {
                    ...
                    // `None` = a daemon with no headless support (bare sbmux) →
...
mod tests {
    ...
    /// Non-sbmux bytes on the socket get a framed error, not a misparse.
    #[tokio::test]
