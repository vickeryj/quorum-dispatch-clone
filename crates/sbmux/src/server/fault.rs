...
//! - `SBMUX_FAULT_PTY_WRITE` = `error` | `swallow`
//! - `SBMUX_FAULT_ERRNO`     = int (default 5 = EIO), for `error` mode
//! - `SBMUX_FAULT_DROP_FRAMES` = `send-input` (park-open frame drop)
//! - `SBMUX_FAULT_SESSION`   = exact session-name filter (default: all)
//! - `SBMUX_FAULT_MATCH_SHA256` = 64-hex content filter (default: all)
//!
...
impl FaultLayer {
    ...
    pub fn from_env() -> Arc<Self> {
        let write_mode = match std::env::var("SBMUX_FAULT_PTY_WRITE").ok().as_deref() {
            Some("error") => Some(WriteFault::Error),
            Some("swallow") => Some(WriteFault::Swallow),
            Some(other) => {
                tracing::warn!(value = %other, "SBMUX_FAULT_PTY_WRITE unrecognized — fault layer NOT armed");
                None
            }
            None => None,
        };
        let errno = std::env::var("SBMUX_FAULT_ERRNO")
            .ok()
            .and_then(|s| s.parse::<i32>().ok())
            .unwrap_or(5); // EIO
        let drop_send_input_frames =
            std::env::var("SBMUX_FAULT_DROP_FRAMES").ok().as_deref() == Some("send-input");
        let session_filter = std::env::var("SBMUX_FAULT_SESSION").ok();
        let sha_filter = std::env::var("SBMUX_FAULT_MATCH_SHA256").ok();
