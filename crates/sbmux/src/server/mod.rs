...
/// `from_secs(5)` to a shared const so the exit-wait bound (`SBMUX_EXIT_WAIT_MS`
...
pub(super) async fn drop_blocking_with_timeout<T: Send + 'static>(value: T, label: &str) {
    ...
    let result = tokio::time::timeout(
        ...
        tokio::task::spawn_blocking(move || {
            // §4.1 CR-3 test-lane seam: when `SBMUX_TEST_SLOW_DROP_MS` is set,
...
/// `SBMUX_TEST_SLOW_DROP_MS` is set to a millisecond count, the BLOCKING drop
...
fn test_slow_drop_injection() {
    if let Some(ms) = std::env::var("SBMUX_TEST_SLOW_DROP_MS")
        .ok()
        .and_then(|s| s.parse::<u64>().ok())
        .filter(|ms| *ms > 0)
    {
...
/// mode is retired.) Env-tunable via `SBMUX_CLEANUP_INTERVAL_MS` (ACK-1 R-REAP
...
fn cleanup_interval_from_env() -> std::time::Duration {
    std::env::var("SBMUX_CLEANUP_INTERVAL_MS")
        .ok()
        .and_then(|s| s.parse::<u64>().ok())
        .map(std::time::Duration::from_millis)
...
/// exit-on-session-end ordering. There is no longer a `<dir>/sbmux.sock` bind path.
pub async fn run_server(
    ...
    // budget checked here). The legacy shared `<dir>/sbmux.sock` bind is gone.
    let path = socket::session_socket_path_for(socket_dir.as_deref(), &session)?;

    // ACK-1: events context (None = SBMUX_EVENTS_DISABLED kill switch) and
    ...
    let ctx = DaemonCtx {
        ...
        // bare sbmux → None). `LaunchHeadless` is refused when None.
        ...
        tokio::spawn(async move {
            ...
            loop {
                if lc_manager.lock().await.is_empty() {
                    ...
                    // drains before process exit, bounded by `SBMUX_EXIT_WAIT_MS`
    ...
    // as the bookend lands. `SBMUX_CLEANUP_INTERVAL_MS` still tunes it (ACK-1
    ...
    // SBMUX_EVENTS_HEARTBEAT_MS. Emits via the same EventWriter path (seq/cap
    // rules apply); post-close emits are no-ops (benign teardown race).
    let heartbeat_handle = std::env::var("SBMUX_EVENTS_HEARTBEAT_MS")
        .ok()
        .and_then(|s| s.parse::<u64>().ok())
        .map(|ms| {
