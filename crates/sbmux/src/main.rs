...
//!
//! Instead of intercepting scrollback (like tmux/screen), sbmux sends completed
//! lines directly to stdout, letting the client terminal handle scrolling natively.
//!
//! This binary wraps the sbmux library for daemon/client operations.

use clap::Parser;
use cli::{Cli, Command};
use sbmux::{cli, client, protocol, server};
use std::io::Read;
...
fn init_tracing() {
    tracing_subscriber::fmt()
        .with_env_filter(
            EnvFilter::try_from_default_env().unwrap_or_else(|_| EnvFilter::new("sbmux=info")),
        )
