...
/// has `SBMUX_TEST_LEAK=retain`, every PTY chunk is ALSO copied into a buffer
...
impl LeakControl {
    fn from_env() -> Option<Self> {
        match std::env::var("SBMUX_TEST_LEAK").ok().as_deref() {
...
/// process env has `SBMUX_TEST_SEAM_DELAY_MS=<site>:<millis>`, a sleep of
...
impl SeamDelay {
    /// Read the hook config from `SBMUX_TEST_SEAM_DELAY_MS`. `None` (inert)
    /// unless the env names a valid `<a|b|c>:<u64>`.
    pub(crate) fn from_env() -> Option<Self> {
        Self::parse(std::env::var("SBMUX_TEST_SEAM_DELAY_MS").ok().as_deref()?)
    }
...
fn persistent_reader_loop(
    ...
    // Leak-guard negative control: None (inert) unless SBMUX_TEST_LEAK is set.
    ...
    // B3 R8 seam-hole probe: None (inert) unless SBMUX_TEST_SEAM_DELAY_MS is set.
    ...
    loop {
        match reader.read(&mut buf) {
            ...
            Ok(mut n) => {
                ...
                // forever. No-op when SBMUX_TEST_LEAK is unset.
