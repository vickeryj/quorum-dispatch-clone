...
//! in its argv. (The sbmux harness spawns daemons directly, so the pid is always
...
//! root (`/tmp/sbmux-runs/*`) for run dirs whose OWNER PID IS DEAD (prior runs);
...
/// The fixed family root all sbmux test jails live under (see `jail::Jail`).
const SBMUX_FAMILY_ROOT: &str = "/tmp/sbmux-runs";
...
/// root and never contend with concurrently-running real sbmux jails.)
pub fn reap_prior_run_daemons(current_run_root: &Path) -> usize {
    reap_in_family(Path::new(SBMUX_FAMILY_ROOT), current_run_root)
}
...
mod tests {
    ...
    fn identity_pinned_reap_negative_and_positive() {
        ...
        // running real sbmux jails (whose setup-reapers scan the real family root)
        // so the controls are deterministic and the test can never disturb — or be
        // disturbed by — a sibling.
        let family = Path::new(SBMUX_FAMILY_ROOT).join(format!("reaper-selftest-{nanos}"));
