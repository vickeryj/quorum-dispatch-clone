...
//!
//! Provides high-level wrappers around sbmux CLI commands and socket-level
...
use sbmux::protocol::codec::{encode, FrameReader};
use sbmux::protocol::{write_preamble, ClientMsg, ConnectMode, ServerMsg};

/// Protocol-level client for direct socket communication with sbmux daemon.
...
impl ProtocolClient {
    /// Connect to the sbmux daemon socket and send the version preamble.
...
/// Path to the sbmux binary under test. `CARGO_BIN_EXE_sbmux` is set by cargo
...
pub fn sbmux_binary() -> PathBuf {
    PathBuf::from(env!("CARGO_BIN_EXE_sbmux"))
}
...
pub fn list_sessions(
    socket_path: &Path,
) -> Result<Vec<sbmux::protocol::SessionInfo>, Box<dyn Error>> {
    let result = std::thread::spawn({
        let socket_path = socket_path.to_path_buf();
        move || {
            let rt = tokio::runtime::Runtime::new().unwrap();
...
/// historical behavior).
///
/// WS-C M3b: spawns a PER-SESSION daemon (`sbmux server --socket-dir <dir>
/// --session <session>`) binding `<dir>/<session>.sock` and returns THAT socket.
/// The legacy shared-daemon mode (no `--session`, bound `sbmux.sock`) is RETIRED
/// (spec §1, §9). A generous default `SBMUX_CLAIM_TIMEOUT_MS` keeps the
/// freshly-spawned (still EMPTY) daemon alive until the test's first
/// session-addressed verb claims it; a caller-supplied `SBMUX_CLAIM_TIMEOUT_MS`
...
pub fn start_daemon_in_jail_with_stderr(
    ...
    if !env_vars.iter().any(|(k, _)| k == "SBMUX_CLAIM_TIMEOUT_MS") {
        env_vars.push(("SBMUX_CLAIM_TIMEOUT_MS".to_string(), "60000".to_string()));
    }
    ...
    tracing::info!(
        "[daemon] starting per-session sbmux server in jail, socket will be at: {}",
        socket_path.display()
    );

    let sbmux_bin = sbmux_binary();

    tracing::debug!("[daemon] using sbmux binary at: {}", sbmux_bin.display());

    // Spawn daemon as a child process (per-session `sbmux server --session`).
    let stderr: Stdio = match stderr_file {
        Some(p) => Stdio::from(std::fs::File::create(p)?),
        None => Stdio::null(),
    };
    let daemon = Command::new(&sbmux_bin)
        .arg("server")
        .arg("--socket-dir")
        .arg(&jail.socket_dir)
        .arg("--session")
    ...
    // Detach: send the daemon to background (setsid happens inside sbmux server)
