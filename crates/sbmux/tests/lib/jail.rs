...
//!
//! Ported from `test/golden/lib/jail.sh` (tag phase-0b-part1) and adapted for sbmux
...
//! relay port, socket prefix. Daemon sockets live ONLY at $XDG_RUNTIME_DIR/sbmux/.
//!
//! Jail setup:
//! 1. Create per-run temp dirs under a sandbox base (sbmux-<runid>-* prefix for safety)
...
//! 2. Verify no live sockets remain under $XDG_RUNTIME_DIR/sbmux/
//! 3. Remove jail_root
...
pub struct Jail {
    ...
    /// Jail root directory (sandbox base: /tmp/sbmux-runs/<runid>/)
    pub jail_root: PathBuf,

    /// Session name prefix for safety: sbmux-<runid>-
    ...
    /// Socket directory for sbmux daemons: $XDG_RUNTIME_DIR/sbmux/
...
impl Jail {
    ...
    pub fn establish(runid: Option<String>) -> Result<Self, Box<dyn std::error::Error>> {
        ...
        // daemon socket at <jail>/xdg_runtime/sbmux/sbmux.sock then exceeds
        ...
        let base = Path::new("/tmp").join("sbmux-runs");
        ...
        let jail = Jail {
            ...
            socket_dir: jail_root_clone.join("xdg_runtime").join("sbmux"),
            real_home,
    ...
    pub fn assert_established(&self) -> Result<(), Box<dyn std::error::Error>> {
        ...
        if !jail_root_str.contains("sbmux-runs/") {
            return Err(format!(
                "JAIL_ROOT '{}' not under sbmux-runs/ sandbox base",
                jail_root_str
            )
            .into());
        }
        ...
        for entry in fs::read_dir(&self.socket_dir)? {
            ...
            if path.extension().is_some_and(|ext| ext == "sock") {
                ...
                if let Some(filename) = path.file_stem() {
                    ...
                    // Try to kill via sbmux as well (may not work, but worth trying)
    ...
    fn _kill_session_by_name(&self, name: &str) -> Result<(), Box<dyn std::error::Error>> {
        ...
        // Try to kill via sbmux kill (may fail if sbmux not available, that's OK)
        let _ = Command::new("sbmux")
            .arg("kill")
            .arg(name)
            .env_clear()
            .envs(self.env_vars())
...
mod tests {
    ...
    fn test_session_prefix_guard() {
        ...
        assert!(session_prefix.contains("sbmux-"));
