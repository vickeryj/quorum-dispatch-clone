...
//! seam, via the env-gated `SBMUX_TEST_SEAM_DELAY_MS` hook (`session.rs`
//! `SeamDelay`).
//!
...
//! M4). Run explicitly:
//!
//!   cargo test -p sbmux --test b3_r8_probe -- --ignored --nocapture
//!
...
fn run_cell(site: &str, delay_ms: u64) -> Result<CellResult, Box<dyn Error>> {
    ...
    env.push((
        "SBMUX_TEST_SEAM_DELAY_MS".to_string(),
