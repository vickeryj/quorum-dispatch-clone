//! WS-C M3a integration tests — the sbmux client-library half of the engine
//! flip (spec §3.2 identity belt, §4.2 launcher four liveness states, §4.3
//! discovery probe). These drive REAL per-session `sbmux server --session
...
use sbmux::client::discovery::scan_sessions;
use sbmux::client::server_launcher::{ensure_session_server_running, ServerLaunchSpec};
use sbmux::client::session_client::connect_session_stream;

/// Build a [`ServerLaunchSpec`] that re-execs the sbmux TEST binary with
/// `["server"]` (the standalone daemon entry — the embedder's `sb sbmux-server`
...
fn ensure_cold_start_then_idempotent() -> Result<(), Box<dyn Error>> {
    ...
    let _claim = EnvGuard::set(&[("SBMUX_CLAIM_TIMEOUT_MS", "60000")]);
    sweep_orphan_daemons();
...
fn ensure_crashed_daemon_respawns_fast() -> Result<(), Box<dyn Error>> {
    ...
    let mut daemon = spawn_daemon_direct(&jail, "alpha", &[("SBMUX_CLAIM_TIMEOUT_MS", "60000")])?;
...
fn ensure_relaunches_after_session_end() -> Result<(), Box<dyn Error>> {
    ...
    block_on(async {
        ...
        send_verb(
            &mut s,
            sbmux::protocol::ClientMsg::CreateDetached {
                name: "alpha".into(),
    ...
    block_on(async {
        ...
        send_verb(
            &mut s,
            sbmux::protocol::ClientMsg::KillSession {
                name: "alpha".into(),
            },
        )
        .await
...
fn identity_belt_rejects_wrong_daemon() -> Result<(), Box<dyn Error>> {
    ...
    let expected = format!(
        "sbmux daemon at {} identifies as session 'alpha', expected 'beta'",
        beta.display()
    );
...
fn scan_stale_socket_unlinked_log_preserved() -> Result<(), Box<dyn Error>> {
    ...
    let mut env_vars = jail_env(&jail);
    env_vars.push(("SBMUX_CLAIM_TIMEOUT_MS".into(), "60000".into()));
    env_vars.push(("PATH".into(), "/usr/bin:/bin".into()));
    env_vars.push(("TERM".into(), "xterm-256color".into()));
    let _ = std::fs::remove_file(&socket);
    let mut daemon = Command::new(sbmux_binary())
        .arg("server")
        .arg("--socket-dir")
        .arg(&dir)
        .arg("--session")
...
/// §4.3 / §5.3: a legacy `sbmux.sock` present is EXCLUDED from the scan — never
/// probed as a session.
#[test]
fn scan_excludes_legacy_sbmux_sock() -> Result<(), Box<dyn Error>> {
    let _serial = SERIAL.lock().unwrap_or_else(|e| e.into_inner());
    ...
    // A live per-session daemon (claimed) AND a planted legacy sbmux.sock file.
    block_on(async {
        ensure_session_server_running(Some(&dir), "alpha", Some(&spec))
            .await
            .expect("ensure failed");
    ...
    let legacy = dir.join("sbmux.sock");
    ...
    assert!(
        legacy.exists(),
        "legacy sbmux.sock must be excluded by name, not probed/unlinked"
    );
...
fn scan_transient_refusal_survives_and_returns_row() -> Result<(), Box<dyn Error>> {
    ...
    let daemon = std::thread::spawn(move || {
        ...
        if cfd >= 0 {
            ...
            let hello = sbmux::protocol::encode(&sbmux::protocol::ServerMsg::Hello {
                caps: vec![],
                session: "alpha".into(),
            })
            .unwrap();
            let list = sbmux::protocol::encode(&sbmux::protocol::ServerMsg::SessionList(vec![
                sbmux::protocol::SessionInfo {
                    name: "alpha".into(),
                    pid: 4242,
                    cols: 80,
                    rows: 24,
...
async fn send_verb(
    ...
    msg: sbmux::protocol::ClientMsg,
) -> Result<sbmux::protocol::ServerMsg, String> {
    use sbmux::protocol::{encode, FrameReader, ServerMsg};
