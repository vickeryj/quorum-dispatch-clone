...
use sbmux::events::{parse_line, CloseReason, DaemonEvent};
use std::error::Error;
use std::fs;
...
fn meta_of(ev: &DaemonEvent) -> &sbmux::events::EventMeta {
    match ev {
...
fn kill_session_via_cli(env: &[(String, String)], session: &str) -> Result<(), Box<dyn Error>> {
    ...
    let status = std::process::Command::new(libmod::client::sbmux_binary())
        .arg("kill")
        .arg(session)
        .env_clear()
        .envs(cmd_env)
        .status()?;
    if !status.success() {
        return Err(format!("sbmux kill '{session}' failed: {status}").into());
    }
    Ok(())
}
...
fn live_row_assertions(
    ...
    match evs.first() {
        Some(DaemonEvent::SessionOpened {
            ...
            if *schema_version != sbmux::events::DAEMON_EVENTS_SCHEMA_VERSION {
...
/// dir as sbmux.sock (<jail>/xdg_runtime/sbmux/events/...).
#[test]
...
/// session-closed(child-exited). Interval lowered via SBMUX_CLEANUP_INTERVAL_MS
...
fn r_reap_child_exited() -> Result<(), Box<dyn Error>> {
    ...
    env.push(("SBMUX_CLEANUP_INTERVAL_MS".into(), "300".into()));
    let (_daemon, _socket) = start_daemon_in_jail(&jail, &env, "ar1")?;
    let session = "ar1";

    let rt = tokio::runtime::Runtime::new()?;
    let cwd = std::fs::canonicalize(&jail.jail_root)?;
    rt.block_on(sbmux::client::session_client::create_detached_session(
...
fn r_rot_cap_truncation() -> Result<(), Box<dyn Error>> {
    ...
    let mut env = jail_env(&jail);
    env.push(("SBMUX_EVENTS_MAX_BYTES".into(), "600".into()));
...
/// R-HB: SBMUX_EVENTS_HEARTBEAT_MS=100 → heartbeat records appear, seq
...
fn r_hb_heartbeat_knob_on() -> Result<(), Box<dyn Error>> {
    ...
    let mut env = jail_env(&jail);
    env.push(("SBMUX_EVENTS_HEARTBEAT_MS".into(), "100".into()));
...
/// R-F1 (injection 2): SBMUX_FAULT_PTY_WRITE=error on session s →
...
fn r_f1_fault_error() -> Result<(), Box<dyn Error>> {
    ...
    let mut env = jail_env(&jail);
    env.push(("SBMUX_FAULT_PTY_WRITE".into(), "error".into()));
    env.push(("SBMUX_FAULT_SESSION".into(), "af_target".into()));
    // WS-C M3b: per-session split — each session gets its OWN daemon. Both
    // daemons inherit the SAME fault env (SBMUX_FAULT_SESSION=af_target), so the
...
fn r_f2_fault_swallow() -> Result<(), Box<dyn Error>> {
    ...
    let mut env = jail_env(&jail);
    env.push(("SBMUX_FAULT_PTY_WRITE".into(), "swallow".into()));
    env.push(("SBMUX_FAULT_MATCH_SHA256".into(), sha_hex(swallowed)));
...
fn r_f3_fault_dropframe() -> Result<(), Box<dyn Error>> {
    ...
    let mut env = jail_env(&jail);
    env.push(("SBMUX_FAULT_DROP_FRAMES".into(), "send-input".into()));
    env.push(("SBMUX_FAULT_SESSION".into(), "ad_target".into()));
    // WS-C M3b: per-session split — two per-session daemons sharing the SAME
    // fault env (SBMUX_FAULT_SESSION=ad_target). The drop-frame fault bites only
...
fn r_arm_loud_warn_on_stderr() -> Result<(), Box<dyn Error>> {
    ...
    env_a.push(("SBMUX_FAULT_PTY_WRITE".into(), "swallow".into()));
    {
...
/// with `SBMUX_FAULT_PTY_WRITE=error` armed on the test session, so the send
...
fn r_priv_armed_fault_stderr_sha_prefix_only() -> Result<(), Box<dyn Error>> {
    ...
    env.push(("SBMUX_FAULT_PTY_WRITE".into(), "error".into()));
    env.push(("SBMUX_FAULT_SESSION".into(), session.into()));
...
/// R-MUT m1: SBMUX_EVENTS_DISABLED=1 → the R-LIVE assertion body MUST FAIL
...
fn r_mut_m1_kill_switch_reds_live_row() -> Result<(), Box<dyn Error>> {
    ...
    let mut env = jail_env(&jail);
    env.push(("SBMUX_EVENTS_DISABLED".into(), "1".into()));
