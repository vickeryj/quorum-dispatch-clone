//! WS-C M4 SBMUX-LEVEL gate arms (spec §7). These drive a REAL per-session
//! `sbmux server --session <name> --socket-dir <dir>` daemon inside a hermetic
...
//!     SHORT `SBMUX_CLAIM_TIMEOUT_MS`: `sb ls`-shaped scan shows NO phantom row
...
use libmod::client::{pid_alive, sbmux_binary, sweep_orphan_daemons, ProtocolClient};
...
use sbmux::protocol::handshake::{PREAMBLE_MAGIC, PROTOCOL_VERSION};
use sbmux::protocol::{encode, ClientMsg, FrameReader, ServerMsg, ERR_EXPECTED_HELLO};
use sbmux::server::client_handler::ERR_SESSION_ENDED;
...
/// in the jail with extra env (e.g. a short `SBMUX_CLAIM_TIMEOUT_MS`), and
...
fn g_coldstart_claim_timeout_no_phantom_then_reaped() -> Result<(), Box<dyn Error>> {
    ...
    let mut daemon = start_single_session_daemon(
        &jail,
        "ghost",
        &[("SBMUX_CLAIM_TIMEOUT_MS".into(), claim_ms.to_string())],
        true,
    )?;
    let socket = daemon.socket.clone();
    let dir = jail.socket_dir.clone();
...
/// `SBMUX_TEST_SLOW_DROP_MS=600` empties the manager then BLOCKS in its drop,
...
fn w2_grace_ended_window_refuses_new_connect_named() -> Result<(), Box<dyn Error>> {
    ...
    let mut daemon = start_single_session_daemon(
        ...
        &[
            ("SBMUX_CLAIM_TIMEOUT_MS".into(), "60000".into()),
            ("SBMUX_TEST_SLOW_DROP_MS".into(), slow_drop_ms.to_string()),
        ],
        true,
    )?;
    let socket = daemon.socket.clone();
...
//
// Construction (spec §7, red-team M4): the SBMUX_TEST_SHARED=1 seam collapses
...
//
// WHY sbmux-level: two sessions on ONE daemon is a world production NEVER builds;
