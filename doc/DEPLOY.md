# Deploying `sb` to the live binary

**Merging to `main` does NOT ship anything.** The `sb` users run is a plain copy of a release
...
cursor fix, faster `sb new`, token counts, all on `main` but invisible to users. If you merged a
...
The deploy target is wherever `sb` resolves on `PATH` — it differs per machine. **Resolve it; never
hardcode a path.**

```sh
LIVE="$(command -v sb)"            # e.g. ~/.local/bin/sb (Linux) or ~/.bun/bin/sb (legacy mac)
echo "deploying over: $LIVE"
```

Known live paths:
- **Linux / Lima dev box:** `~/.local/bin/sb` (canonical; the `~/.bun/bin/sb` symlink and the TS
  engine were deleted 2026-06-09 — see ADR 0016).
- **macOS (legacy):** `~/.bun/bin/sb`, with `~/.local/bin/sbr` as a canonical rust copy.
...
./scripts/build-lock.sh cargo build --release -p sb     # -> target/release/sb

# 2. Resolve the live path and sanity-check the fresh binary against the LIVE fleet.
LIVE="$(command -v sb)"
target/release/sb ls                    # expect the new behavior + your real sessions
...
case "$(uname)" in Darwin) cp target/release/sb ~/.local/bin/sbr && codesign --force --sign - ~/.local/bin/sbr ;; esac

# 6. Verify.
sb --version && sha256sum "$LIVE" 2>/dev/null | cut -c1-16 || shasum -a 256 "$LIVE" | cut -c1-16
```
...
every new invocation ("Killed: 9", exit 137; `sb ls` silently prints nothing); on Linux an
...
user`), so the relay must be registered there pointing at the deployed `sb`:

```sh
claude mcp add -s user relay -- "$(command -v sb)" relay:serve
...
`~/.claude/relay/<pid>.json` and listens on `127.0.0.1:8900+`; `sb send:relay <session> <msg>`
returns a message id and the target goes busy.

> NOTE (2026-06-10): `sb bootstrap` / `sb relay:repoint` currently write the relay entry to
