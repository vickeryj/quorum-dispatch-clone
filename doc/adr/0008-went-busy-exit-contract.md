# ADR 0008: `sb new -p` went-busy EXIT CONTRACT — a three-way exit divergence from TS
...
`sb new -p <prompt>` boots a session, then DELIVERS the prompt via the
...
TS collapses all three into a SUCCESS exit. `sb new -p` exits 0 whether the
...
warning, but **useless to an external composer** — `sbx spawn` shells out to
`sb new -p` and must branch on the exit code alone. "Created and running" and
"created but the prompt may be stuck" are operationally different outcomes; a
single exit 0 cannot express the difference.

## Decision

`sb new` with `-p` defines a THREE-WAY exit contract (a Class-4-style deliberate
divergence, ADR 0007 — the hardening IS the deliverable). It is documented in
THREE places that move together: this ADR, `doc/PROTOCOL.md`, and the `sb new
--help` epilogue.
...
file is an INFRA failure — the registry row that `sb ls` / `sb attach` key on is
gone, so the session is not reliably addressable. Routing it to 10 ("created,
prompt maybe unsubmitted, go attach it") would tell `sbx spawn` to attach a
...
clap's default exit-2 convention survives — `bin/sb/main.rs:16-17` documents this,
`bin/sb/config.rs` asserts the config-usage 2s), 3 (`ping` no-arg validation,
...
- External composition (`sbx spawn`, scripts) can branch on the exit code alone:
