...
Two pieces of `qd bootstrap` were still shaped around the retired bun/TS
...
1. **The relay step offered a dead external installer.** The ADD-5 design
   ...
   target command as `<home>/.bun/bin/qd` — on a machine where that symlink
...
2. **The shell wrapper was a fossil by design.** The TS bootstrap WROTE a
   ...
   baked block survived unmanaged — and when the Rust `qd new` changed its
...
- `relay:repoint` (and the bootstrap registration) target
  **`std::env::current_exe()`** — the binary being invoked is by definition
  the deployed one. No more hardcoded `.bun/bin/qd`.
- `apply_repoint` **creates `~/.claude/.mcp.json` when absent** (fresh-machine
  registration); `MigrateError::McpConfigAbsent` is removed.

**Shell: the eval-init pattern, via a new `qd init <shell>` verb.**
`qd init bash|zsh|fish` PRINTS the integration (the `claude` wrapper +
ZMX_DIR pin); the rc file carries one stable line
(`eval "$(qd init bash)"` / zsh variant / `qd init fish | source` in
`~/.config/fish/conf.d/qd.fish`). The wrapper body ships in the binary
(`qd::shell_init`), so it can never drift from the engine's verb contracts —
...
(`>>> qd bootstrap >>>` markers) with a remove-it pointer — it never edits
existing rc content.
...
patching is not engine bootstrap's job: with the TS/qb deploy layer deleted
(2026-06-09), the wrapper's only owner is the engine, and the wrapper's
content is engine behavior (`qd new` routing). The G-B5 forbidden-token set
drops `profile-patch|profile patch` accordingly; the qb-vocabulary tokens
(substrate/marketplace/plugins-root/spawn/qb) stay banned as wording
discipline. The bun/TS implementation (repo, global package, `.bun/bin/qd`
...
- Fresh-machine setup is one consented `qd bootstrap`: state dirs, relay
...
- `qd relay:repoint` run from a dev build registers THAT dev build
