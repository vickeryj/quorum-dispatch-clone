...
| Hermetic paths | `<JAIL_ROOT>/qd_home` → `<QD_HOME>`, `/zmx` → `<ZMX_DIR>`, `/xdg_*` → `<XDG_*>`, jailed `/tmp` → `<TMPDIR>`, residual `<JAIL_ROOT>` | tokens | Suffix after the dir (e.g. `zmx-<uid>` resolution structure) is preserved — it carries the Bug-D resolution-order meaning. **DISTINCTNESS (W1 P6c):** an UNJAILED host `/tmp/…` path (a jail-escape) → distinct `<HOST_TMP>`, NOT `<TMPDIR>` (see §W1). |
...
| Run ids / session prefix | `qdrg-<runid>-` → `qdrg-<RUNID>-`; bare `<runid>` → `<RUNID>` | tokens | |
